...
- 当 Todolist 启用 Redis 时，Redis 为唯一数据来源。todo_create/todo_modify 后 Agent 将 Core TodoTool 落盘的数据同步到 Redis；Agent 从 Redis 读取 Todolist 用于事件发射和状态检查。
- 文件系统（`.todo/<sessionId>/todo.json`）保留作为 Core TodoTool 的落盘介质，供 Core 后续 todo_modify 使用；读取优先级为 Redis → 文件 → 内存缓存。
...
- Todolist Redis 化验收必须验证：写入/读取/跨实例共享/多实例隔离。
...
- 测试必须覆盖 Todolist Redis 写入/读取/跨实例共享/多实例 key 隔离。
