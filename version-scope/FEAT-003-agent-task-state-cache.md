...
| Todolist Redis 持久化  | MUST   | 新增     | AgentCore DeepAgent 的 Todolist 增加 Redis 作为持久化数据源，当以 Redis 为持久化数据源时，使分布式无状态部署下同一会话可跨实例共享 Todolist 上下文，Redis 为唯一数据源。                    |
| Todolist 读写能力       | MUST   | 新增     | AgentCore 必须提供 Todolist 的写入（save）和读取（load）能力，使用同一 sessionId 作为 key 标识，保证分布式部署下跨实例读取的数据一致性。                                            |
| Todolist Key 隔离     | MUST   | 新增     | AgentCore Todolist Redis key 必须使用 `{keyPrefix}:todo:{rawSessionId}` 格式，支持多实例共用 Redis 时通过 keyPrefix 隔离；集群模式下 key 不得包含花括号（hash tag 安全）。 |
...
| Todolist Key 前缀配置   | configuration    | 应支持配置 Todolist Redis key 前缀（`todo.key-prefix`），用于多实例共用 Redis 时的 keyspace 隔离。                                                       |
| Todolist 持久化数据源配置   | configuration    | 应支持通过配置控制 Todolist 持久化数据源为文件或 Redis，默认文件。配置为文件时 Todolist 使用文件系统兼容原版本，配置为 Redis 时 Redis 为唯一权威数据来源。                                  |
...
### 4.2 Todolist 持久化数据源配置场景（新增）

AgentCore DeepAgent 提供 Todolist 持久化能力，持久化数据源为 Redis 和文件二选一。配置 Todolist 采用 Redis 作为持久化数据源时，AgentCore 内部完成 Redis 读写和数据同步，上层 Agent 使用 Todolist 时不感知 Redis 的存在，调用接口保持一致。

| 场景                  | 前置条件                                            | AgentCore 提供的能力                                                                    | Agent 如何使用                                                                           |
| ------------------- | ----------------------------------------------- | ---------------------------------------------------------------------------------- | ------------------------------------------------------------------------------------ |
| Todolist 文件模式（默认）   | 配置 Todolist 持久化数据源为文件（默认）                       | AgentCore `TodoTool` 提供文件读写能力，数据存储在 `.todo/<sessionId>/todo.json`                  | Agent 通过 Core `TodoTool` 接口读写 Todolist，不感知存储细节。                                      |
| Todolist Redis 模式   | 配置 Todolist 持久化数据源为 Redis                       | AgentCore 内部完成 Redis 读写（SET with TTL），数据存储在 Redis                                  | Agent 通过 Core `TodoTool` 接口读写 Todolist，不感知 Redis 存在。                                 |
| Todolist 跨实例共享      | 配置 Todolist 持久化数据源为 Redis，分布式部署，同一会话被路由到不同实例    | AgentCore 内部通过 Redis 共享 Todolist，多实例可共用同一 Redis 数据源                                | Agent 实例 B 通过 Core `TodoTool` 接口读取实例 A 写入的 Todolist，获取完整任务列表和执行状态；跨实例状态一致，不感知 Redis。 |
| Todolist 多实例 Key 隔离 | 配置 Todolist 持久化数据源为 Redis，多个 Agent 实例共用同一 Redis | AgentCore 通过 `keyPrefix` 配置实现 keyspace 隔离，key 格式 `{keyPrefix}:todo:{rawSessionId}` | 部署方为不同实例配置不同 `keyPrefix`，Agent 不感知 key 格式细节。                                         |
...
- 当配置 Todolist 持久化数据源为 Redis 时，AgentCore 以 Redis 为 Todolist 唯一权威数据来源，所有读写均走 Redis。AgentCore 内部完成 Redis 读写和数据同步，上层 Agent 通过 Core `TodoTool` 接口使用 Todolist，不感知 Redis 的存在。配置为文件时，AgentCore 使用文件模式，所有读写均走文件系统。
- Todolist 持久化数据源为 Redis 和文件二选一，Redis 模式下不回落文件系统，文件模式下不涉及 Redis。
- Todolist Redis key 格式为 `{keyPrefix}:todo:{rawSessionId}`，使用原始 sessionId（不转义），支持空格/中文/超长 sessionId；集群模式下 key 不含花括号（hash tag 安全）。
- Todolist TTL 采用与 Checkpointer 统一的 TTL 策略（见 §5.1.3），不单独配置。
...
- Todolist 持久化验收必须验证：配置为文件时文件模式正常 / 配置为 Redis 时写入/读取/跨实例共享/多实例 key 隔离。
...
| Todolist 文件系统完全移除         | 不承诺完全移除文件系统存储；文件模式仍作为默认持久化方案，与 Redis 模式二选一。                                                              |
...
- L2 设计必须明确 Todolist Redis 持久化的 key 格式和 TTL 策略；不得把 Todolist 文件系统存储和 Redis 存储合并为同一存储抽象。
- 测试必须覆盖 Todolist 持久化数据源：配置为文件时文件模式正常 / 配置为 Redis 时写入/读取/跨实例共享/多实例 key 隔离。
