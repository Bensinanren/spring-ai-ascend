...
FEAT-005 定义 `agent-runtime` 在部署或启动阶段代表 Agent 访问智能体中间件服务的当前版本事实。当前版本纳入范围的子特性是 **Skill Hub 代理**：runtime 读取部署态 runtime 配置和 Agent skill 选择配置，通过 Skill Hub SPI 访问 Skill Hub，下载 Agent 声明需要的 skill 包，并把可注册的 skill 材料交给 `agent-core` 或框架适配入口完成注册和使用。启动阶段因外部依赖导致下载失败时，runtime 允许 Agent 以 skill 不可用的降级状态进入 ready，并在请求链路外继续完成该 skill 的首次生效。

本特性解决的问题是：Agent 的 skill 资产由外部 Skill Hub 管理，Agent 部署时只声明需要哪些 skill；runtime 需要在 Agent 启动阶段发起连接认证、访问控制、下载、完整性校验、诊断和移交，并在下载故障降级后继续完成首次生效，避免 Agent 开发者在业务代码中直接耦合 Skill Hub 服务 API 或凭据处理。

本特性不是运行中动态 skill 调度或热刷新能力，也不是 `agent-core` 的 skill 解析、注册、执行或 prompt 组装规范。runtime 不在用户请求链路中触发 skill 下载，不解释 skill 包内容，不把 skill instructions 直接注入运行时 prompt，不接管框架内部 tool/skill 选择和执行，也不建立独立于 Skill Hub 的 skill 授权模型。下载故障后异步完成尚未成功注册的 skill 首次生效，不属于热刷新。
...
| 能力 | 要求级别 | 事实要求 |
|---|---|---|
| 部署/启动阶段代理访问 | MUST | runtime 必须在 Agent 部署或启动阶段读取配置并发起 Skill Hub 访问；下载失败后的重试必须在用户请求链路外执行，当前版本不在用户 query 请求过程中触发 skill 下载。 |
...
| skill 包完整性校验 | MUST | runtime 必须在移交前使用 Skill Hub 通过可信通道提供的摘要或等价完整性元数据校验下载材料，采用 SHA-256 或安全性不低于其的散列算法；校验不通过的材料不得移交、注册或使用。 |
| 注册材料移交 | MUST | runtime 必须把下载成功且通过完整性及基本格式校验的 skill 材料移交给 `agent-core` 或框架适配入口；注册、解析、执行和模型上下文处理归属 `agent-core` 或具体框架。 |
| required skill 失败策略 | MUST | required skill 的配置、认证、授权、查找或启动阶段移交失败时，Agent 不得进入 ready；仅下载失败以及因下载不完整、损坏或摘要不匹配导致的完整性校验失败按降级首次生效规则处理。 |
| 下载失败降级与首次生效 | MUST | skill 包下载失败或下载材料完整性校验失败时，runtime 不得把该 skill 暴露为可用，但不得因此阻塞 Agent 进入 ready；runtime 必须在请求链路外继续重试，成功下载、校验并移交注册后，该 skill 从下一轮新请求开始可用。 |
| optional skill 降级 | SHOULD | optional skill 的配置、认证、授权、查找或移交失败时可以跳过该 skill 并继续启动，但必须输出脱敏、可诊断的降级信息；下载或完整性校验失败必须按下载失败降级与首次生效规则处理，被跳过或尚未首次生效的 skill 不得注册为可用。 |
...
| required skill 下载失败 | Agent 声明 required skill，Skill Hub 暂时不可达或下载中断 | runtime 启动 Agent | runtime 不注册该 skill，输出脱敏降级诊断并在请求链路外继续重试；Agent 可进入 ready，但该 skill 暂不可用。 |
| 降级 skill 首次生效 | Agent 已因下载失败进入 ready，runtime 后续成功下载 skill | runtime 校验完整性并移交注册 | 当前进行中的请求不受影响；成功注册后的下一轮新请求可以使用该 skill，后续不自动刷新、替换或卸载。 |
| skill 包完整性校验失败 | 下载材料的摘要与 Skill Hub 通过可信通道提供的摘要不一致，或等价完整性校验不通过 | runtime 校验下载材料 | runtime 拒绝移交和注册该材料，输出脱敏安全诊断并继续获取有效材料；Agent 可保持降级 ready。 |
...
- 当前版本在 Agent 部署或启动阶段发起 skill 获取；下载失败后的重试在用户请求链路外执行，不由用户请求触发。
- required skill 的配置、认证、授权、查找或启动阶段移交失败时，Agent 不应进入 ready；skill 包下载失败或下载材料完整性校验失败时，Agent 可以在该 skill 不可用的降级状态进入 ready。
- runtime 必须持续暴露降级状态和脱敏诊断；成功下载、校验并移交注册后，该 skill 从下一轮新请求开始首次生效，不改变已经开始处理的请求。
- 延迟首次生效只适用于尚未成功注册的 skill；首次成功注册后，当前版本不自动刷新、替换或卸载该 skill。
...
- Skill Hub adapter 必须通过受认证保护的可信通道提供 skill 包及其摘要或等价完整性元数据；runtime 必须使用 SHA-256 或安全性不低于其的散列算法验证下载材料完整性，并可以继续执行格式或可移交性校验，但不解释 skill 的业务语义。
- 完整性元数据缺失、摘要不匹配或等价完整性校验失败时，runtime 必须拒绝移交和注册该材料，并按下载失败降级与首次生效规则继续获取有效材料。
...
| 下载不完整、包损坏或完整性校验失败 | 不得移交或注册对应 skill；Agent 可保持降级 ready，runtime 必须输出脱敏安全诊断并继续获取有效材料。 |
| 注册材料移交失败 | 不得把 skill 暴露为可用；启动阶段已下载成功但移交失败时，required skill 场景 fail fast；Agent 已降级 ready 后的异步移交失败不得影响进行中的请求，runtime 必须保持该 skill 不可用并继续诊断和重试。 |
...
| 边界 | 当前版本不承诺 |
|---|---|
| 请求级动态获取 | 不在每次用户 query 请求时按用户、租户或上下文动态访问 Skill Hub 获取 skill；下载失败后的重试由 runtime 在请求链路外执行。 |
| 运行中热刷新 | 下载故障后完成尚未成功注册 skill 的首次生效不属于热刷新；首次成功注册后，不承诺运行中自动刷新、热替换、卸载或按策略切换 skill。 |
...
- required skill 的配置、认证、授权、查找或启动阶段移交失败必须阻断 Agent ready；下载失败或下载材料完整性校验失败不得阻断 Agent ready，但必须保持该 skill 不可用并在请求链路外继续重试。
- 下载故障后成功获取的 skill 必须先通过完整性校验并完成移交注册，才能从下一轮新请求开始首次生效；不得改变进行中的请求，首次生效后不得自动刷新、替换或卸载。
- 实现不得把 skill instructions 直接注入运行时 prompt，不得绕过 `agent-core` 或框架适配入口解释 skill 内容。
- 测试必须覆盖默认 openJiuwen Skill Hub 对接、自定义 SPI 替换、凭据缺失/无效、`401/403`、skill 不存在、required 非下载类失败阻断 ready、required/optional 下载失败降级、请求链路外重试、完整性元数据缺失或摘要不匹配时拒绝注册、成功下载后的下一轮请求首次生效、首次生效后不热刷新和日志脱敏。
