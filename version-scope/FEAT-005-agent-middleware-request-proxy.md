...
FEAT-005 定义 `agent-runtime` 在部署态读取 Agent 配置，并通过智能体中间件请求代理从 SkillHub 获取该 Agent 所需 skill 的统一接入要求。该特性使 Agent 的模型、提示词、工具、rail、skill 来源和运行时中间件能力可以从部署配置或配置中心声明，而不是固化在应用代码中；运行时基于部署态 Agent 标识和稳定中间件上下文向 SkillHub 请求可见技能，并把技能安装到具体 Agent 框架可消费的位置。
...
本特性对客户的价值是：Agent 能力可以随部署环境、租户和业务场景进行配置化选择，技能目录可以由客户或平台统一治理。对产品的价值是：通过窄 SPI 和请求代理边界接入 SkillHub，避免在各 Agent adapter 中重复实现技能发现、加载和安装逻辑，为后续 Nacos/注册中心、远端 SkillHub 和多租户权限过滤留下稳定扩展点。
...
- 客户平台团队：提供 SkillHub、配置中心或本地目录等技能来源，并按部署环境、租户、Agent 或业务场景过滤技能。
- Runtime 开发者：通过统一 SkillHub SPI 与 adapter 安装钩子接入技能，不把 A2A 执行链路绑定到具体技能仓库。
- Agent 开发者：继续使用具体 Agent 框架的原生 skill/tool 能力，同时接受 runtime 在启动、创建或稳定安装点注入的部署态技能。
...
| ReAct prompt 可见性 | main 分支存在 `runtime_skillhub` prompt section 注入 | 当前需求不把完整 instructions 注入运行时 prompt 作为承诺；运行时应保持渐进加载，只让轻量摘要可见，完整定义按需读取。 |
| 自动装配 | 存在 `SkillHubProvider` 和 OpenJiuwen handler 时自动注入 installer | 无 SkillHub Provider 时 Agent 正常启动；有 Provider 时运行时自动接入请求代理。 |
| 稳定上下文过滤 | Provider 接收 `AgentExecutionContext` | SkillHub 可以基于部署态租户、agentId 或业务场景返回不同 skill 集合；当前版本不要求基于每次请求的 user/session/task 动态注册和取消注册 skill。 |
...
| SkillHub Provider SPI | MUST | 系统必须提供框架中立的 SkillHub SPI，支持列出当前稳定上下文可见的 skill 摘要并按 skillId 加载完整定义。 |
| 渐进式 skill 加载 | MUST | Runtime 不应在摘要阶段加载大量 instructions 或 skill 包；完整定义只在安装或使用前按需加载。 |
| 稳定上下文透传 | MUST | Runtime 请求 SkillHub 时必须携带可稳定获得的部署态上下文，例如 tenant、agentId 和配置来源；当前版本不要求依赖每次 server request 的 user、session 或 task 动态过滤 skill。 |
| OpenJiuwen skill 安装 | MUST | 当前版本必须支持把 SkillHub 返回的 OpenJiuwen 本地 skill 路径安装到 OpenJiuwen ReActAgent 或 DeepAgent 可消费位置。 |
| 完整 instructions 渐进加载 | MUST | Runtime 不得在注册或安装 skill 时把完整 instructions 默认注入运行时 prompt；只能暴露轻量摘要或由框架原生 skill 机制在需要时读取完整定义，避免重复加载和上下文膨胀。 |
...
| 配置与请求代理解耦 | MUST | 部署态 Agent 配置读取和 SkillHub 请求代理必须保持边界清晰：配置负责声明 Agent 与来源，SkillHub 负责按稳定部署上下文返回 skill 内容。 |
...
| SkillDefinition | data model | 必须表达 skillId、name、description、instructions、referenceUris、toolDependencies 和 metadata，用于按需读取完整定义、安装 skill 路径或加载 skill 包；完整 instructions 不应默认注入运行时 prompt。 |
...
| 远端 SkillHub 返回可见技能 | 应用注册 SkillHubProvider，Provider 可访问远端目录 | Runtime 在 Agent 启动、创建或执行前的稳定安装点调用 `listSkills(context)` | Provider 按部署态 tenant、agentId 或业务场景过滤，返回轻量摘要；无可见 skill 时 Agent 正常执行。 |
| 按 skillId 加载完整定义 | SkillHub 摘要返回 skillId | Runtime 在安装或框架需要使用 skill 前调用 `loadSkill(context, skillId)` | Runtime 按需获取 instructions、referenceUris、toolDependencies 和 metadata，并交给 adapter 安装或使用。 |
| OpenJiuwen ReAct skill 安装 | SkillDefinition metadata 包含 OpenJiuwen 本地路径 | Runtime 调用 OpenJiuwen adapter 安装逻辑 | Adapter 调用原生 skill 注册能力；运行时不把完整 instructions 默认注入 prompt，完整内容由原生 skill 机制按需读取。 |
...
- `listSkills(context)` 返回当前稳定部署上下文可见的轻量摘要；Provider 可以根据租户、Agent 或业务场景过滤结果。
- 当前版本不要求 Provider 基于每次 server request 的 user、session 或 task 动态变更 skill 集合；如业务确需请求级权限过滤，应由网关、业务 Provider 预筛选或独立 Agent/Handler 实例承接，避免频繁注册和取消注册 skill。
...
- 对 ReActAgent，runtime 不应在安装阶段默认注入完整 skill instructions；只允许通过原生 skill 描述、轻量摘要或等价索引让模型知道可用 skill，完整 instructions 应在框架需要使用该 skill 时再读取。
...
- 日志必须包含非敏感上下文和计数信息，例如 tenantId、agentId、summary 数量、loaded 数量、installed 数量和 skip reason；不要求记录 user/session/task 级动态过滤结果。
...
- L2 设计必须明确 SkillHub Provider 的调用时机、异常处理、空结果语义、缓存责任和稳定部署上下文过滤约定；如果未来引入请求级过滤，必须单独说明生命周期和性能边界。
- 实现必须保证未装配 SkillHub Provider 时不影响 Agent 启动和请求执行。
- 实现必须保证 OpenJiuwen adapter 在启动、Agent 创建或执行前的稳定安装点安装 SkillHub skills，且与 MCP installer、远程 Agent tool installer 的顺序和冲突语义可诊断。
- 测试必须覆盖合法配置装配、非法 schema 快速失败、环境变量缺失、filesystem skill 展开、重复 skill 冲突、Provider 不存在降级、Provider 返回摘要与定义、OpenJiuwen skill path 安装、完整 instructions 不默认注入 prompt、DeepAgent skill runtime 缺失跳过和日志脱敏。
