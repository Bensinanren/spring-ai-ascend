...
registry-discovery-center 对经过授权的业务调用方公开 `DiscoverAgentCards`，用于查询满足显式约束的逻辑 Agent Card 候选。Agent Card 的注册、更新与下线由部署发布事实接入、标准 Agent Card 抓取和注册中心内部对账自动完成。
...
| `RegistryRequestContext` | request context | 调用方执行 Agent Card 发现时必须携带统一治理上下文，至少包含 `tenantId`、`callerRef`、`traceId`、`requestId` 和 `deadline`。 |
...
| `AgentCardDiscoveryFailure` | discovery failure | 发现接口失败时返回统一失败结构，至少包含 `failureCode`、可编程说明、`retryable` 和 `traceId`。当前版本必须区分 `INVALID_QUERY`、`CALLER_NOT_AUTHORIZED`、`TENANT_SCOPE_DENIED`、`DEADLINE_EXCEEDED` 和 `REGISTRY_UNAVAILABLE`。 |

### 3.2 Agent Card 主动注册协作入口

| 入口 / 契约 | 方向 | 事实要求 |
|---|---|---|
| `DeploymentDiscoveryProvider` | registry-discovery-center 调用 provider | 作为部署发布事实源的可替换接入 SPI，必须支持读取当前实例全量快照和监听实例增量事件。 |
| `ListDeploymentInstances` | registry-discovery-center 调用 provider | 注册中心在启动、周期校验、事件中断或 source revision 缺口后拉取指定 provider 的权威全量快照，用于补充、刷新和撤销 Agent Card 发布来源。 |
| `WatchDeploymentInstances` | registry-discovery-center 调用 provider | provider 按 source revision 提供 `ADDED`、`MODIFIED`、`TERMINATING` 和 `DELETED` 等增量事件；事件不可恢复或 revision 不连续时，注册中心转入全量 reconciliation。 |
| `DeploymentInstanceObservation` | provider 返回 registry-discovery-center | 表达 provider 观察到的单个 Agent Service 发布实例，至少包含 `tenantId`、`serviceId`、`instanceId`、内部 `baseUrl`、部署版本、ready / terminating 状态、`sourceId`、`sourceRevision` 和 `observedAt`。`tenantId + serviceId + instanceId` 构成发布事实对账身份。 |
| `GET /.well-known/agent-card.json` | registry-discovery-center 调用 agent-runtime | 注册中心依据可信 `DeploymentInstanceObservation.baseUrl` 调用 Agent Service 的标准 A2A Agent Card 地址，并对响应执行来源、schema、协议和安全校验。 |
| 标准 A2A `AgentCard` | agent-runtime 返回 registry-discovery-center | 作为 Agent 身份、A2A 能力声明和调用协议元数据的标准载体。Card 中可提供给调用方的地址必须是平台批准的逻辑入口；provider 内部 `baseUrl` 只用于可信抓取。 |
