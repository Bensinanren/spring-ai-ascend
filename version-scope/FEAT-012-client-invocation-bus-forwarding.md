...
本节描述 Gateway 通过 Event Bus 转发客户端调用时使用的事件封装和投影契约，不是 Gateway 对业务 client 暴露的外部接口。客户端仍面对 FEAT-2026-006 / FEAT-2026-011 定义的统一 client facade / Gateway A2A facade；Event Bus 事件只在 Gateway、Event Bus、worker / agent-service 和 runtime 之间流转。
...
| `INVOCATION_ACCEPTED` | runtime / worker -> Event Bus 投影事件 | 表示 runtime / 目标服务已接受调用，必须携带 taskId、correlation 和幂等结果。 |
| `INVOCATION_STREAM_READY` | runtime / worker -> Event Bus 投影事件 | 表示流可订阅，携带 streamRef 或桥接所需引用，不携带 token、SSE frame 或大对象正文。 |
| `INVOCATION_INPUT_REQUIRED` | runtime / worker -> Event Bus 投影事件 | 表示 runtime Task 进入等待输入状态，必须携带 taskId、correlation 和输入需求描述；Gateway 只负责向 client 呈现该新状态。 |
| `INVOCATION_TERMINAL` | runtime / worker -> Event Bus 投影事件 | 表示任务 completed、failed、canceled 或 rejected；Task 终态仍由 runtime 拥有。 |
| payloadRef / artifactRef | 事件引用字段 | 指向大载荷或结果，不要求 Event Bus 保存正文。 |
| clientInvocationId | Gateway 侧关联字段 | 由 client 携带或由 Gateway 生成 / 复用，用于入队、状态观察和幂等恢复，不能替代 taskId。 |
