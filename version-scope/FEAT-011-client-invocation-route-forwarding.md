...
FEAT-2026-011 定义 `agent-gateway` 当前版本作为客户端统一 A2A 调用路由转发入口的事实：Gateway 必须接收 `agent-client` 封装后的 A2A 兼容 C/S 请求，完成认证鉴权、租户识别、请求标准化、路由决策和 HTTP/SSE 桥接，再把请求投递到目标 agent-service 或 runtime 标准入口。

本特性解决的问题是：业务应用不应感知内部 agent-service、runtime 物理 endpoint，也不应自行选择未治理 URL。Gateway 作为 agent-bus 中的入口治理平面，必须根据 agentId、tenant、版本、健康状态和路由策略生成 routeHandle，并将 A2A 调用、流式观察、查询、取消和订阅请求按路由转发回目标 Task owner。
...
| 统一 A2A 封装转发 | MUST | Gateway 必须以统一 A2A 兼容请求表面承接 `SendMessage`、`SendStreamingMessage`、`GetTask`、`CancelTask` 和 `SubscribeToTask` 等语义，并转发到目标 runtime `/a2a` 或受控 agent-service facade；不得把 invoke 与 stream 作为互相独立的客户端入口协议。 |
| SSE 桥接 | MUST | Gateway 必须支持对目标服务 A2A SSE 的桥接，但不得生成 Agent token 内容，也不得把流式观察变成独立于 A2A Task 的第二套接口。 |
...
| 入口 | 类型 | 事实要求 |
|---|---|---|
| Gateway A2A facade | HTTP/SSE 入口 | 接收 agent-client 封装后的 A2A 兼容请求；同一入口承载阻塞调用、流式调用、Task 查询、取消和重订阅语义，不分别暴露独立 invoke facade 与 stream facade。 |
| `SendMessage` / `SendStreamingMessage` | A2A 调用语义 | Gateway 基于 agentId、tenant、user、session、correlation、trace 和幂等键完成入口治理和路由转发；runtime 或目标服务负责创建和推进 Task。 |
| `GetTask` / `CancelTask` / `SubscribeToTask` | A2A Task 控制语义 | Gateway 只基于服务端 taskId、routeHandle、route trace 或亲和记录定位 Task owner 并转发；clientInvocationId 只能作为网关侧关联信息，不能替代 taskId。 |
| A2A SSE | 服务流 | Gateway 只桥接目标服务 A2A SSE，不生成 token、不缓存完整流、不决定 Task 终态。 |
...
| 流式调用桥接 | 目标支持 A2A SSE / streaming，client 连接仍保持 | client 通过同一 A2A facade 发起 `SendStreamingMessage` | Gateway 转发调用并在目标返回可订阅流后桥接 A2A SSE；实时 token 内容由目标服务产生，Gateway 不生成、不缓存 Task 终态。 |
...
- Gateway facade、agent-service 和 runtime A2A 入口必须共享 routeHandle、tenant、taskId、streamRef、幂等键和错误码语义；clientInvocationId 只能作为网关侧关联句柄，不得替代 taskId。
