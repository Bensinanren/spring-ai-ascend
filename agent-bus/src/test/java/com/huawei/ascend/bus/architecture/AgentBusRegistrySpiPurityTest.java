 ...
 * ({@code com.huawei.ascend.bus.spi.registry..}).
 *
 * <p>Symmetric to {@link AgentBusSpiPurityTest} (which guards the rest of the
 * agent-bus SPI surface) and {@link AgentBusForwardingSpiPurityTest} (which
 * guards the forwarding substrate). The registry SPI is the
 ...
 */
class AgentBusRegistrySpiPurityTest {

    /**
     * Production registry SPI classes only
     * ({@code com.huawei.ascend.bus.spi.registry} and sub-packages). Test
    ...
    void spi_registry_does_not_import_spring() {
        noClasses().that().resideInAPackage("com.huawei.ascend.bus.spi.registry..")
