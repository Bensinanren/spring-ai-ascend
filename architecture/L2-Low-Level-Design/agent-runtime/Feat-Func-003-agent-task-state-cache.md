...
应用启动
  ...
  │     └─ cluster: 使用 nodes，database 必须为 0
  ├─ 输出数据源策略日志（脱敏）
...
| cluster database 非 0 | `type=cluster` 且配置了非 0 database | 启动失败或 Redis 使用方创建失败 | 明确提示 Redis Cluster 只支持 db 0。 |
...
| cluster nodes 校验 | `type=cluster` 时 nodes 必填，database 必须为 0。 |
...
| 启动日志 | 输出 redis-ref、endpoint type、实际 `RuntimeRedisClient` Bean、非敏感连接摘要、密码是否配置；不得输出明文密码或加密密文。 |
| 测试 | 覆盖单机默认 Bean、集群默认 Bean、多节点配置校验、客户自定义 Bean 覆盖、日志脱敏、TaskStore 和 checkpointer 复用 SPI。 |
