...
| 能力                      | 状态 | 说明                                                                                |
| ------------------------- | ---- | ----------------------------------------------------------------------------------- |
| 声明本地 skill 路径       | 草案 | 通过 `openjiuwen.service.agent.skills.sources[].type=local` 声明一个或多个本地目录        |
...
* **必须**：`type` 不在 `{local, github}` 枚举内时抛"不支持远端来源"诊断，立即失败而非降级处理
...
`SkillSourceInstaller.create()` 在工厂方法内执行校验，校验失败抛 `IllegalArgumentException`（对齐 review 意见，不引入自定义异常）：
...
│   └── SkillSourceProperties.java                 # 新增：@ConfigurationProperties(prefix="openjiuwen.service.agent.skills")
└── external/
...
AgentCoreExtAutoConfiguration  →  SkillSourceProperties
  │  @EnableConfigurationProperties   (@ConfigurationProperties prefix=openjiuwen.service.agent.skills
 ...
 * @since 2026-07-12
 */
@ConfigurationProperties(prefix = "openjiuwen.service.agent.skills")
...
```
application.yml
  openjiuwen.service.agent.skills.sources[]
  │
  ▼ Spring 绑定
...
| type 缺失            | `sources[].type` 为 null             | 抛 `IllegalArgumentException` | 启动失败，异常 message 含索引定位             |
| local path 为空      | `type=local` 且 `path` 为 blank       | 抛 `IllegalArgumentException` | 启动失败                                       |
| local path 不存在    | 路径不存在                            | warn，不阻断              | Agent 正常启动，该路径 skill 不可用            |
| github owner 格式错  | `owner` 含 `/` 或为空                 | 抛 `IllegalArgumentException` | 启动失败                                       |
| directory 路径逃逸   | `directory` 含 `../` 或绝对路径        | 抛 `IllegalArgumentException` | 启动失败                                       |
| 不支持远端来源       | `type` 非 `local`/`github`            | 抛 `IllegalArgumentException` | 启动失败，message 含 `unsupported remote source type` |
...
# 或 application.yml:
# openjiuwen.service.agent.skills.sources[2].credential-ref 解析为 ${GH_TOKEN:}
```
...
| 属性路径                                                 | 类型   | 默认值 | 说明                                                          |
| -------------------------------------------------------- | ------ | ------ | ------------------------------------------------------------- |
| openjiuwen.service.agent.skills.skill-mode               | String | all    | skill 模式：all/auto_list/none，透传给 `SkillUseRail.skillMode` |
| openjiuwen.service.agent.skills.sources[].type           | String | —      | 来源类型：`local` \| `github`（必填）                         |
| openjiuwen.service.agent.skills.sources[].path           | String | —      | local: 本地目录路径（local 必填）                             |
| openjiuwen.service.agent.skills.sources[].owner          | String | —      | github: 仓库 owner，单段不含 `/`（github 必填）               |
| openjiuwen.service.agent.skills.sources[].repo           | String | —      | github: 仓库名，单段不含 `/`（github 必填）                   |
| openjiuwen.service.agent.skills.sources[].ref            | String | HEAD   | github: 分支/tag/commit                                       |
| openjiuwen.service.agent.skills.sources[].directory      | String | (空)   | github: 仓库内子路径，禁止 `../` 和绝对路径                   |
| openjiuwen.service.agent.skills.sources[].credential-ref | String | (空)   | github: 凭据引用名，运行时解析为 token                         |
