...
L2 详细设计是在对应模块 L1 高阶设计指导下，按特性颗粒度展开实现级设计。它回答“某个已接受特性如何由当前代码实现、由哪些接口承诺、由哪些配置驱动、由哪些测试或样例验证”，不重新定义 L0/L1 已经确定的系统边界、模块职责或跨模块依赖方向。
...
- 与特性直接相关的接口契约、类协作、配置项、错误语义、运行流程和验证入口。
...
以下材料不是当前 L2 架构事实，保留在 `docs/` 下作为草稿、提案或验证指南：

| 材料 | 位置 | 状态 | 说明 |
|---|---|---|---|
| 中间件解耦样例配置与端到端验证指南 | `docs/architecture/l2/agent-runtime/drafts/middleware-decoupling-examples.cn.md` | draft | 面向测试团队和客户开发团队的样例配置与验证说明。 |
