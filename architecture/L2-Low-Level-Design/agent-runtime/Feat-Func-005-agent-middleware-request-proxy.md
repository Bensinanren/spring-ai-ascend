...
5. **三层分离** — `SkillHubProvider`（SPI：start/download/verify/stop）负责数据源无关的 skill 访问；`SkillHubManager`（管理器）编排下载/校验/注册三阶段 + 后台重试 + 标识位；`SkillHubInstaller`（普通类）负责注册到 agent 实例（框架相关）
...
9. **下载失败后台重试**（PR #415）— 下载/校验失败时 Agent 降级为 ready，skill 不可用；`SkillHubManager` 启动后台线程定时重试 download，成功后校验并置标识位=true；后台线程只负责 download + verify + 维护标识位，不触碰 SkillManager（注册严格在请求线程执行）
...
| SkillHub 管理器 | 编排下载/校验/注册三阶段；后台重试；标识位管理 | `SkillHubManager` | ✅ |
...
| 后台重试 | 下载失败时由 SkillHubManager 启动后台线程定时重试，成功后校验并置标识位 | `SkillHubManager` 内部后台线程 | ✅ |
...
| Agent skill 选择配置驱动 | Agent 声明 `skills:[{id,version,required}]` 驱动获取 | — | ⬜ 第一期不做，待注册列表包含全部下载校验通过的 skill |
...
| required 下载/校验失败降级 | ✅ MUST | required skill 的下载或完整性校验失败时 Agent 降级为 ready，skill 不可用；SkillHubManager 启动后台线程定时重试下载，成功后校验并置标识位=true |
...
| Skill 待注册列表 | ✅ | `SkillHubManager` 维护"待注册列表"（download + verify 后校验通过的路径）和标识位；`Handler.query()`/`streamQuery()` 阶段调 `register(agent)` 委托 `SkillHubInstaller` 注册 |
...
| Agent skill 选择配置驱动 | ⬜ | 第一期不做 Agent 声明 `skills:[{id,version,required}]` 驱动；`SkillHubManager` 的"待注册列表"包含全部下载校验通过的 skill，按其 `required` 字段决定 fail fast / 降级行为 |
...
| Agent skill 选择配置驱动（第一期） | 第一期保持 `SkillHubManager` 的"待注册列表"包含全部下载校验通过的 skill 的模型；Agent 声明 `skills:[{id,version,required}]` 驱动推迟到后续版本 | `LocalSkillEntry` 只含 skillId + localPath，required 由 Provider 实现按部署态决定 |
 ...
 * 构造入参为 SkillHubProvider 实现 bean。
 *
 * 生命周期：
 *   构造方法 → provider.start() + 触发首次下载
 *     首次下载成功 → verify 校验下载的 skill → 校验通过项加入"待注册列表" → 标识位=true
 *     首次下载失败 → 启动后台线程定时重试 download，成功后 verify + 置标识位=true
 *   Handler.query/streamQuery → 调 register(agent) → 从"待注册列表"取路径，委托 SkillHubInstaller.installBeforeStart(agent, paths)
 *   Manager.stop() / Handler.stop → provider.stop() + 停后台线程
 *
 * 线程安全约束：
 *   - 后台线程只负责 download + verify + 维护"待注册列表"与标识位，不触碰 agent-core 的 SkillManager
 *   - 注册严格在 query/streamQuery 请求线程执行，规避 SkillManager 非线程安全
 *
 * 标识位语义：
 *   - true 表示有需要注册的 skill（待注册列表非空）
 *   - false 表示无需要注册的 skill（下载中/下载失败/校验失败/无 skill）
 *   - 标识位 false 时请求照常处理，只是 skill 尚未注册（skill 不可用但 Agent 可响应）
 */
public class SkillHubManager {

    /** 构造方法：入参为 SkillHubProvider 实现 bean + properties + decryptor。内部调用 provider.start() 并触发首次下载。 */
     ...
     * 下载成功 → verify 校验 → 校验通过项加入"待注册列表" → 标识位=true
     * 下载失败 → 启动后台线程定时重试，成功后校验并置标识位=true
     */
    public void download() { ... }

    /**
     * 注册 skill 到 agent 实例（用于 Handler.query/streamQuery 阶段）。
     * 仅当标识位=true 时执行：从"待注册列表"取路径，委托 SkillHubInstaller.installBeforeStart(agent, paths)。
     * 标识位=false 时直接返回（请求照常处理，skill 尚未注册）。
     * 首次成功注册后标识位置 false，避免重复注册。
     */
...
> 模块/包归属：`agent-service-adapters-agentcore-ext`，包 `com.openjiuwen.service.adapters.agentcore.ext.middleware.skillhub`（普通类，非契约）。

`SkillHubInstaller` 是将 skill 路径注册到 agent 实例的普通工具类，由 `SkillHubManager.register()` 委托调用：

```java
/**
 * 将 skill 路径注册到 agent 实例的普通工具类。
...
public class SkillHubInstaller {
     ...
     * @param skillPaths  待注册的 skill 本地路径列表（由 SkillHubManager 的"待注册列表"提供）
     */
    public void installBeforeStart(Object agent, List<Path> skillPaths) { ... }
...
- **必须**：`download()` 成功后逐条调用 `provider.verify(skillPath)`，校验通过的路径加入"待注册列表"，置标识位=true；校验失败的路径不加入
- **必须**：`download()` 失败时启动后台线程定时重试 `provider.download()`，成功后执行 verify + 置标识位=true
- **必须**：后台线程只负责 download + verify + 维护"待注册列表"与标识位，不触碰 agent-core 的 SkillManager
- **必须**：`register(agent)` 仅在标识位=true 时执行，从"待注册列表"取路径委托 `SkillHubInstaller.installBeforeStart(agent, paths)`；标识位=false 时直接返回
- **必须**：`register(agent)` 首次成功注册后置标识位=false，避免重复注册
- **必须**：`Handler.start()` 阶段调用 `SkillHubManager.download()`；`Handler.query()`/`streamQuery()` 阶段调用 `SkillHubManager.register(agent)`
- **必须**：标识位=false 时 `query`/`streamQuery` 请求照常处理，只是 skill 尚未注册（skill 不可用但 Agent 可响应）
- **必须**：`SkillHubInstaller.installBeforeStart(agent, paths)` 仅在 `agent instanceof DeepAgent` 或 `agent instanceof BaseAgent` 时执行注册
- **必须**：下载材料完整性校验由 `SkillHubProvider` 实现自决（不强制算法），校验失败由 `verify` 返回 false 或抛 `IllegalStateException`（message 前缀 `SkillHub[CHECKSUM_MISMATCH]`），Manager 据此将失败项从"待注册列表"排除
- **必须**：日志输出 tenantId、agentId、installed 数量、skipped 数量、是否降级（具体 skillId、failureCategory 由 Provider 内部日志输出）
- **必须**：required skill 的移交失败（`registerSkill` 后 skillCount 未增长）时 `installBeforeStart` 抛异常
...
- **禁止**：在每次请求时重复注册同一 skill（首次成功注册后置标识位=false）
        ...
        └── src/main/java/com/openjiuwen/service/adapters/agentcore/ext/
            ...
            └── middleware/                        # 新增：实现层（对齐 runtime agent-service-adapters 分层）
                └── skillhub/                      # skillhub 特性子包（对齐 runtime middleware/redis）
                    ...
                    ├── SkillHubInstaller.java     # 安装器普通类（对齐 RemoteA2aToolInstaller 模式）
                    ├── SkillHubManager.java       # 管理器：编排 download/verify/register + 后台重试 + 标识位
...
AutoConfiguration                    │
  │ @ConditionalOnProperty           │ 构造方法(provider, properties, decryptor)
  │   openjiuwen.service.            │   → provider.start() + 触发首次 download()
  │   middleware.skillhub.enabled    ├─ download() 成功 → 逐条 verify(skillPath) → 待注册列表 + 标识位=true
  │ @ConditionalOnMissingBean        │   download() 失败 → 后台线程定时重试 + verify + 标识位=true
  │   SkillHubProvider               │
  │  默认 = OpenJiuwen               │ register(agent)（query/streamQuery 阶段调）
  │  SkillHubProvider                │   标识位=true → 取待注册列表 → 委托 SkillHubInstaller.installBeforeStart(agent, paths)
  │                                  │   标识位=false → 直接返回（请求照常处理，skill 尚未注册）
  ▼                                  │   首次成功注册后置标识位=false
SkillHubProvider                ←────┤
(OpenJiuwen 默认                     │ stop() → 停后台线程 + provider.stop()
 或 业务 @Bean 覆盖)                 │
 ┌──────────────────────────┐        │ 分层失败语义：
 │ start(properties, decryptor)│     │   required 移交失败 → throw
 │ download(properties, decryptor)│   │   optional 任何失败 → warn + skip
 │   → boolean              │        ▼
 │ verify(skillPath)        │   SkillHubInstaller
 │   → boolean              │  .installBeforeStart(agent, paths)（请求线程串行注册）
 │ stop()                   │   注：校验方式由 Provider 实现自决，
 │                          │      文档不约束具体校验算法
 └──────────────────────────┘
...
JiuwenCoreAgentExtHandler (extends JiuwenCoreAgentHandler)
  ...
  │         └─ Runner.start()
  │
  └─ streamQuery / query  ← override：调 skillHubManager.register(agent)（标识位=true 时注册，false 时照常处理）
...
请求处理（streamQuery / query）
  ...
  │     │  │                                                             │
  │     │  ├─ if (标识位 == false) → 直接返回（请求照常处理，skill 尚未注册）
  │     │  ├─ if (标识位 == true) {
  │     │  │     paths = 待注册列表
  │     │  │     skillHubInstaller.installBeforeStart(agent, paths)
  │     │  │       ├─ agent instanceof DeepAgent → innerAgent = deepAgent.getAgent()
  │     │  │       ├─ agent instanceof BaseAgent → target = baseAgent
  │     │  │       ├─ 其他 → warn + skip
  │     │  │       └─ for each path: target.registerSkill(path)
  │     │  │     首次成功注册后置标识位=false（避免重复注册）
  │     │  │   }
  │     │  └─────────────────────────────────────────────────────────────┘
  │
...
- required skill 移交失败（registerSkill 后 skillCount 未增长）→ `installBeforeStart` 抛异常 → 请求线程感知
- required skill 下载/校验失败 → 降级（标识位=false），`start()` 继续 `super.start()`，Agent ready（skill 不可用）；后台线程重试成功后置标识位=true
- optional skill 任何失败 → warn + skip + 降级诊断，该 skill 不注册为可用
- 下载/校验失败的 skill 不得加入"待注册列表"
- 标识位=false 时请求照常处理，skill 尚未注册；首次成功注册后置标识位=false 避免重复注册
...
skill path 从 `SkillHubManager` 的"待注册列表"获取（download + verify 后校验通过的路径）。path 是本地文件系统路径（解压后根目录或 zip 文件），直接传给 `BaseAgent.registerSkill(path)`。

### 4.4 完整性校验（由 Provider 实现自决）

依据 PR #415：runtime 必须在移交前校验下载材料。但**校验方法由 `SkillHubProvider` 实现自决**（SHA-256 / 常规文件检查 / 自定义均可），本设计文档不强制约束具体校验算法。唯一要求：校验失败的材料不得注册为可用，由 `verify(skillPath)` 逐条返回 false 或抛出 `IllegalStateException`（message 前缀 `SkillHub[CHECKSUM_MISMATCH]`，分类见 §4.10），`SkillHubManager` 据此将失败项从"待注册列表"排除。
...
SkillHubMiddlewareAutoConfiguration
  ...
  │   → new SkillHubManager(provider, properties, decryptor)
...
- **required skill 移交失败阻断请求**：`installBeforeStart` 抛异常，请求线程感知（start() 阶段下载失败不阻断，降级 + 后台重试）
- **required skill 下载/校验失败不阻断 start()**：降级（标识位=false）后继续 `super.start()`，进入 external 注册和 `Runner.start()`；后台线程重试成功后置标识位=true，后续请求注册 skill
...
| required | verify | `CHECKSUM_MISMATCH` | **degrade**（失败项从待注册列表排除） | ✓ 降级 ready |
...
| `CHECKSUM_MISMATCH` | 完整性校验失败 | **降级**（失败项从待注册列表排除） | 降级 skip |
...
由 `SkillHubManager` 的"待注册列表"（download + verify 后校验通过的路径）产出。
...
public String streamQuery(/* 现有参数 */) {
    if (skillHubManager != null) {
        skillHubManager.register(getAgent());  // 标识位=true 时注册，false 时直接返回
    }
...
- `query()`/`streamQuery()` 阶段 `register(agent)` 在标识位=true 时执行注册；标识位=false 时直接返回，请求照常处理
- `register(agent)` 内部 `installBeforeStart` 移交失败时抛异常，请求线程感知
...
`SkillHubProvider.download(properties, decryptor)` 下载应下载的全部 skill 到 `properties.getLocalDir()` 本地目录。`SkillHubManager` 扫描该目录获取已下载的 skill 路径，逐条调 `SkillHubProvider.verify(skillPath)` 校验，校验通过的路径加入"待注册列表"。目录结构由 Provider 实现自决（如按 skillId 子目录组织），但最终注册的路径必须是 `BaseAgent.registerSkill(path)` 可直接使用的本地路径。
...
INFO  SkillHub skill verified skillId=hotel-booking verified=true   ← Provider 内部日志
INFO  SkillHub ready to register=true   ← Manager 层（标识位置 true）
```
...
5. 下载成功 → Manager 扫描本地路径，逐条调 `provider.verify(skillPath)` → 校验通过路径加入"待注册列表"，置标识位=true
6. 下载失败 → Manager 启动后台线程定时重试 `provider.download()`，成功后 verify + 置标识位=true
7. required 下载/校验失败 → 降级（标识位=false）+ 后台重试；optional 失败 → skip
8. `super.start()` → `Runner.start()`
9. 请求处理 → `Handler.query()`/`streamQuery()` 调 `skillHubManager.register(agent)` → 标识位=true 时 `installBeforeStart(agent, paths)` → `Runner.runAgentStreaming` → agent 使用已注册的 skill
10. 首次成功注册后置标识位=false，避免重复注册
...
| 场景 | 期望行为 |
|---|---|
| 启动时获取 required skill | runtime 通过 SkillHub SPI 认证访问，下载 skill 包，完整性校验通过后加入"待注册列表"；query/streamQuery 时注册；Agent ready |
| required skill 下载/校验失败 | Agent 降级为 ready，skill 不可用；SkillHubManager 启动后台线程定时重试下载，成功后校验并置标识位=true |
...
| 下载/校验失败后台重试 | 日志告警 skill 不可用；Agent 继续运行（已降级 ready）；后台重试成功后置标识位=true，后续请求注册 skill |
| 标识位=false 时请求 | 请求照常处理，skill 尚未注册（Agent 可响应但无 skill 可用） |
| 首次成功注册后重复请求 | 首次注册后置标识位=false，后续请求不重复注册 |
...
| # | 测试场景 | 验证点 | 预期结果 |
|---|---|---|---|
| T1 | required skill 下载失败（config/auth/lookup） | 降级 + 后台重试 | `start()` 正常返回，`Runner.start()` 已调用；标识位=false；后台线程启动重试 |
| T2 | required skill 下载失败（download） | 降级 + 后台重试 | 同 T1；skill 未注册；后台重试成功后置标识位=true |
| T3 | required skill 校验失败（checksum mismatch） | 降级，失败项从待注册列表排除 | 同 T2，且 `verify` 返回 false 或抛 `CHECKSUM_MISMATCH`，失败路径不加入"待注册列表" |
| T4 | 下载成功后注册 | 请求期注册 | `download` 返回 true；`verify` 返回 true；标识位=true；`query()`/`streamQuery()` 调 `register(agent)` 后 `registerSkill` skillCount 增长；请求可用到 skill |
| T5 | 校验通过（Provider 用 SHA-256） | digest 校验 | `verify` 返回 true；路径加入"待注册列表"；skill 注册成功（校验方式由 Provider 自决，文档不强制 SHA-256） |
| T6 | 校验通过（Provider 用常规检查） | 常规校验兜底 | 同 T5（校验方式由 Provider 自决） |
| T7 | 校验失败拒绝注册 | 不注册未校验材料 | `verify` 返回 false 或抛 `CHECKSUM_MISMATCH`；失败路径不加入"待注册列表"；required 降级 / optional skip |
| T8 | 首次有效注册后不重复注册 | 幂等保护 | 首次 `register(agent)` 注册后置标识位=false，后续请求不重复注册 |
...
| T14 | DeepAgent 适配 | 取 inner ReActAgent | `installBeforeStart(DeepAgent, paths)` 取 `deepAgent.getAgent()` 注册 |
| T15 | 标识位=false 时请求 | 请求照常处理 | 标识位=false 时 `query()`/`streamQuery()` 直接调父类，skill 尚未注册但 Agent 可响应 |
| T16 | 后台重试成功后注册 | 标识位置 true | 下载失败后后台重试成功 → verify 通过 → 置标识位=true → 后续请求 `register(agent)` 注册 skill |
...
| `SkillManager` 非线程安全（已确认） | 已核实 agent-core `SkillManager` 的 `registry`（`LinkedHashMap`）/`updateAtCache`（`LinkedHashMap`）/`skillOrder`（`ArrayList`）均非线程安全集合，`register`/`refreshIncrementally`/`clearAll` 等方法无同步保护 | `SkillHubManager` 后台线程只负责 download + verify + 维护标识位，不触碰 SkillManager；注册严格在 `query()`/`streamQuery()` 请求线程执行，规避并发写 |
