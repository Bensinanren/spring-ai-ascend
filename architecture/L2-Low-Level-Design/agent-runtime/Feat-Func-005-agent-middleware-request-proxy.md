...
5. **SPI 分离** — `SkillHubProvider`（下载校验 + 本地列表）负责数据源无关的 skill 获取，`SkillHubInstaller` 负责安装到 agent 实例（框架相关）
...
9. **下载失败降级**（PR #415）— 下载/校验失败时 Agent 降级为 ready，skill 不可用；不在请求链路外重试，规避 SkillManager 非线程安全
...
| SkillHub SPI | 下载校验 + 列表接口（`SkillHubProvider`） | `SkillHubProvider` | ✅ |
...
| 后台重试执行器 | 已移除——下载/校验失败降级即可，不再请求链路外重试 | ~~`SkillHubRetryExecutor`~~（已删除） | ✅ |
...
| skill 包下载 | ✅ MUST | `SkillHubProvider.downloadAndVerify(properties, decryptor)` 下载应下载的全部 skill 到 properties.getLocalDir() 本地目录，校验由 Provider 实现自决，返回 boolean（成功/失败） |
...
| required 下载/校验失败降级 | ✅ MUST | required skill 的下载或完整性校验失败时 Agent 降级为 ready，skill 不可用；不在请求链路外重试 |
...
| Skill 摘要列表 | ✅ | `SkillHubProvider.listSkills(properties)` 入参 properties，扫描 properties.getLocalDir() 下已下载+已存在的 skill 返回 `List<LocalSkillEntry>`，每项只含 skillId + localPath |
...
| Agent skill 选择配置驱动 | ⬜ | 第一期不做 Agent 声明 `skills:[{id,version,required}]` 驱动；`listSkills(properties)` 返回本地全部 skill，按其 `required` 字段决定 fail fast / 降级行为 |
...
| Agent skill 选择配置驱动（第一期） | 第一期保持 `listSkills(properties)` 返回本地全部 skill 的模型；Agent 声明 `skills:[{id,version,required}]` 驱动推迟到后续版本 | `LocalSkillEntry` 只含 skillId + localPath，required 由 Provider 实现按部署态决定 |
...
```java
/**
 * SkillHub Provider：从 Skill Hub 下载 skill 到本地指定目录并校验 + 列举本地已下载/已存在的 skill。
 *
 * 职责一：下载校验——入参是配置文件中配置的 SkillHub 连接参数 + 解密密文 token 需要的方法；
 *         Provider 内部自行从 Skill Hub 拉取该配置下应下载的全部 skill，下载到 properties.getLocalDir() 指定目录并校验；
 *         校验方式由实现自决（文档不约束具体算法），但校验失败的材料不得落盘使用。
 * 职责二：列表——把下载到本地的 skills 以及本地已经存在的 skill 以列表形式返回，
 *         列表中的项只包括 skillId 和本地路径，其他信息不要求。
 */
public interface SkillHubProvider {

    /**
     * 下载应下载的全部 skill 到 properties.getLocalDir() 本地目录并校验。
     ...
     *
     * @param properties  SkillHub 连接配置（endpoint、authType、encryptedToken、localDir 等，来自 runtime middleware 配置）
     * @param decryptor   凭据解密器（实现内部调用 decryptor.decrypt(properties.getEncryptedToken()) 解密 token）
     * @return true 表示下载校验全部成功；false 表示部分或全部失败（具体哪些失败由日志记录，调用方后续调 listSkills(properties) 只能拿到已落盘的 skill）
     */
    boolean downloadAndVerify(SkillHubMiddlewareProperties properties,
                              CredentialDecryptor decryptor);

    /**
     * 扫描本地已下载 + 已存在的 skill，返回列表。
     * 本地目录从 properties.getLocalDir() 获取。
     *
     * @param properties  SkillHub 连接配置（含 localDir）
     * @return 本地 skill 列表，每项含 skillId + localPath；无 skill 时返回空列表
     */
    List<LocalSkillEntry> listSkills(SkillHubMiddlewareProperties properties);
}
```
...
**校验职责说明**：skill 下载后如何校验是具体 `SkillHubProvider` 实现的事情，本设计文档不强制约束校验算法（SHA-256 / 常规文件检查 / 自定义均可）。唯一要求：校验失败的材料不得返回成功结果（抛异常即可，分类见 §4.10）。
...
public class SkillHubInstaller {
     ...
     * Installer 内部持有 properties + decryptor + provider。
     *
     * 分层失败语义（PR #415）：
     *   - required skill 配置/认证/查找/移交失败 → 抛异常，调用方据此阻断 Runner.start()，Agent 不 ready
     *   - required skill 下载/完整性校验失败 → 降级（不抛异常），skill 不可用，Agent 可 ready
     *   - optional skill 任何失败 → 内部捕获并记录降级诊断，不抛异常，被跳过的 skill 不得注册为可用
     */
    public void installBeforeStart(Object agent) { ... }
...
- **必须**：skill path 从 `LocalSkillEntry.localPath()` 获取（由 `listSkills(properties)` 扫描本地返回）
- **必须**：下载材料完整性校验由 `SkillHubProvider` 实现自决（不强制算法），校验失败由 `downloadAndVerify` 返回 false 或抛 `IllegalStateException`（message 前缀 `SkillHub[CHECKSUM_MISMATCH]`），Installer 捕获后按 required/optional 语义处理
- **必须**：`SkillHubProvider.listSkills(properties)` 返回 null 时按空列表处理，不抛异常
...
- **必须**：required skill 的下载/校验失败时 `installBeforeStart` 不抛异常，Agent 降级 ready，skill 不可用
...
│       └── skillhub/
│           ├── spi/
│           │   └── SkillHubProvider.java            # 下载校验 + 列表接口：downloadAndVerify / listSkills（便于整体替换）
│           ├── dto/
...
AutoConfiguration                    │
  │ @ConditionalOnProperty           │ installBeforeStart(agent)
  │   openjiuwen.service.            │   入参：agent + properties + decryptor
  │   middleware.skillhub.enabled    ├─ agent instanceof DeepAgent
  │ @ConditionalOnMissingBean        │   ? → install(deepAgent.getAgent())
  │   SkillHubProvider               │   : instanceof BaseAgent
  │  默认 = OpenJiuwen               │       ? → install(baseAgent)
  │  SkillHubProvider                │       : → skip (warn)
  │                                  │
  ▼                                  │ 分层失败语义：
SkillHubProvider                ←────┤   required 配置/认证/查找/移交失败 → throw
(OpenJiuwen 默认                     │   required 下载/校验失败 → degrade（skill 不可用）
 或 业务 @Bean 覆盖)                 │   optional 任何失败 → warn + skip
 ┌──────────────────────────┐
 │ listSkills(properties)   │        ▼
 │   → List<LocalSkillEntry>│   SkillHubInstaller
 │   (扫描 properties.getLocalDir())
 │                          │  .registerSkill(agent, path)（启动期串行注册）
 │ downloadAndVerify(       │
 │   properties, decryptor) │   注：校验方式由 Provider 实现自决，
 │   → boolean              │      文档不约束具体校验算法
 │   (下载到 properties.getLocalDir())
 └──────────────────────────┘
...
JiuwenCoreAgentExtHandler (extends JiuwenCoreAgentHandler)
  │
  ├─ start()   ← override
  │    ├─ skillHubInstaller.installBeforeStart(agent)   ← 新增
  │    │     throw → 不调用 super.start()，Agent 不 ready
  │    │     degrade → 继续调用 super.start()，skill 不可用
  │    └─ super.start()
...
JiuwenCoreAgentExtHandler.start()
  │
  ├─ try { skillHubInstaller.installBeforeStart(agent) }
  │     │   入参隐含：properties（middleware 配置）+ decryptor（凭据解密器）
  │     │
  │     │  ┌─────────────────────────────────────────────────────────────┐
  ...
  │     │  │       └─ no → warn, skip (return)                            │
  │     │  │                                                             │
  │     │  ├─ try { downloadAndVerify(properties, decryptor) → boolean ok
  │     │  │     ok=false → required degrade；optional warn+skip
  │     │  │   } catch (CONNECT/AUTH/NOT_FOUND/ACCESS_DENIED) → required throw / optional skip
  │     │  │   catch (DOWNLOAD/CHECKSUM) → required degrade / optional skip
  │     │  │
  │     │  ├─ entries = provider.listSkills(properties)
  │     │  │     扫描 properties.getLocalDir() 下已下载 + 已存在的 skill
  │     │  │
  │     │  ├─ for each entry in entries:                                 │
  │     │  │   try {                                                      │
  │     │  │     target.registerSkill(entry.localPath())                 │
  │     │  │     log: installed skillId, localPath                        │
...
**关键语义**：
- `installBeforeStart` 对下载和注册分别 try/catch
- required skill 配置/认证/查找/移交失败 → 抛异常向上传播 → `start()` 不调用 `super.start()`，Agent 不 ready
- required skill 下载/校验失败 → 降级（不抛异常）→ `start()` 继续 `super.start()`，Agent ready（但失败的 skill 不可用）
- optional skill 任何失败 → warn + skip + 降级诊断，该 skill 不注册为可用
- 下载/校验失败不重试，skill 不可用
...
skill path 从 `LocalSkillEntry.localPath()` 获取（由 `listSkills(properties)` 扫描本地已下载+已存在的 skill 返回）。path 是本地文件系统路径（解压后根目录或 zip 文件），直接传给 `BaseAgent.registerSkill(path)`。

### 4.4 完整性校验（由 Provider 实现自决）

依据 PR #415：runtime 必须在移交前校验下载材料。但**校验方法由 `SkillHubProvider` 实现自决**（SHA-256 / 常规文件检查 / 自定义均可），本设计文档不强制约束具体校验算法。唯一要求：校验失败的材料不得落盘使用，由 `downloadAndVerify` 返回 false 或抛出 `IllegalStateException`（message 前缀 `SkillHub[CHECKSUM_MISMATCH]`，分类见 §4.10），Installer 捕获后按 required/optional 语义处理。
...
SkillHubMiddlewareAutoConfiguration
  ...
  │   → new SkillHubInstaller(provider, properties, decryptor)
  │
...
- **required skill 下载/校验失败不阻断**：降级（skill 不可用）后继续 `super.start()`，进入 external 注册和 `Runner.start()`
...
第一期 `LocalSkillEntry` 只含 skillId + localPath，无 required 字段。required/optional 语义由 Provider 实现内部决定（如从配置或 Skill Hub 元数据推断），Installer 通过 Provider 提供的 required 标记执行分层失败语义。后续版本可通过 `SkillHubMiddlewareProperties` 配置 required skill 列表。

**失败行为矩阵**（PR #415 更新）：

| skill 类型 | 失败阶段 | 错误分类 | 行为 | Agent ready |
|---|---|---|---|---|
| required | downloadAndVerify（内部连接） | `CONNECT_FAILED` | **throw**（fail fast） | ✗ 阻断 |
| required | downloadAndVerify（内部连接） | `AUTH_FAILED` | **throw**（fail fast） | ✗ 阻断 |
| required | downloadAndVerify | `NOT_FOUND` | **throw**（fail fast） | ✗ 阻断 |
| required | downloadAndVerify | `ACCESS_DENIED` | **throw**（fail fast） | ✗ 阻断 |
| required | downloadAndVerify | `DOWNLOAD_FAILED` | **degrade**（skill 不可用） | ✓ 降级 ready |
| required | downloadAndVerify | `CHECKSUM_MISMATCH` | **degrade**（skill 不可用） | ✓ 降级 ready |
...
| `DOWNLOAD_FAILED` | 下载中断、包损坏 | **降级**（skill 不可用） | 降级 skip |
| `CHECKSUM_MISMATCH` | 完整性校验失败 | **降级**（skill 不可用） | 降级 skip |
| `INSTALL_FAILED` | registerSkill 后 skillCount 未增长 | fail fast | 降级 skip |
| `UNSUPPORTED` | downloadAndVerify 不支持（如 Skill Hub 无 artifact 下载能力） | fail fast | 降级 skip |
...
由 `SkillHubProvider.listSkills(properties)`（扫描 properties.getLocalDir() 下已下载+已存在 skill）产出。
...
public void start() {
    try {
        // Installer 内部持有 properties + decryptor + provider
        skillHubInstaller.installBeforeStart(getAgent());
    } catch (Exception ex) {
        // required skill 非下载失败（config/auth/lookup/handover）时阻断 Agent ready
        // 下载/校验失败已在 installer 内部降级处理，不会传播到此处
        log.error("SkillHub install failed, agent not ready reason={}", sanitize(ex.getMessage()));
        throw ex;
    }
...
- `installBeforeStart` 降级返回时（required 下载/校验失败 或 optional 失败），`start()` 继续 `super.start()`，Agent ready（降级 skill 不可用）
...
2. 当 `enabled=true` 时创建 `OpenJiuwenSkillHubProvider`（实现 `SkillHubProvider`）、`SkillHubInstaller` bean
3. 通过 `@Autowired(required = false)` 注入到 `JiuwenCoreAgentExtHandler`
4. `start()` 时自动执行 skill 下载+校验+注册

### 7.2 本地 skill 目录约定

`SkillHubProvider.downloadAndVerify(properties, decryptor)` 下载应下载的全部 skill 到 `properties.getLocalDir()` 本地目录。`SkillHubProvider.listSkills(properties)` 扫描该目录 + 已存在的 skill 返回列表。目录结构由 Provider 实现自决（如按 skillId 子目录组织），但返回的 `LocalSkillEntry.localPath()` 必须是 `BaseAgent.registerSkill(path)` 可直接使用的本地路径。

### 7.3 日志输出示例

**正常安装**：
```
INFO  SkillHub install started tenantId=default agentId=hotel-agent
INFO  SkillHub skill download/verify succeeded skillId=hotel-booking   ← Provider 内部日志
INFO  SkillHub skill registered skillId=hotel-booking skillCountBefore=0 skillCountAfter=1   ← Installer 层
INFO  SkillHub install completed tenantId=default agentId=hotel-agent installed=2 skipped=0
```

**required skill 下载失败降级**：
```
WARN  SkillHub skill download/verify failed skillId=hotel-booking required=true category=DOWNLOAD_FAILED   ← Provider 内部日志
INFO  SkillHub install completed (degraded) tenantId=default agentId=hotel-agent installed=1 skipped=0   ← Installer 层（汇总，不输出具体 skillId）
```

**optional skill 跳过**：
```
WARN  SkillHub skill download/verify failed skillId=weather-lookup required=false category=DOWNLOAD_FAILED reason=download-interrupted   ← Provider 内部日志
INFO  SkillHub install completed tenantId=default agentId=hotel-agent installed=1 skipped=1   ← Installer 层（汇总）
```
...
2. `JiuwenCoreAgentExtHandler.start()` 调用 `installBeforeStart(agent)`
3. Installer 调用 `provider.downloadAndVerify(properties, decryptor)` → boolean（Provider 内部自决下载哪些 skill，下载到 properties.getLocalDir() + 校验由 Provider 自决）
4. Installer 调用 `provider.listSkills(properties)` → 扫描本地已下载+已存在 skill，获取 `List<LocalSkillEntry>`
5. Installer 对 `listSkills(properties)` 返回的每个 `LocalSkillEntry` 调用 `registerSkill(localPath)`（启动期，主线程）
6. required 下载/校验失败 → 降级（skill 不可用）；required 非下载失败 → throw；optional 失败 → skip
7. `super.start()` → `Runner.start()`（若未 throw）
8. 请求处理 → `Runner.runAgentStreaming` → agent 使用已注册的 skill
...
| SPI 方法 | HTTP API | 说明 |
|---|---|---|
| `listSkills(properties)` | 无（扫描 properties.getLocalDir() 目录） | 扫描本地已下载 + 已存在的 skill，返回 `List<LocalSkillEntry>`，每项含 skillId + localPath |
| `downloadAndVerify(properties, decryptor)` | `GET /api/v1/plugins?plugin_type=skill` + `GET /api/v1/artifacts/{id}?version={ver}` | 先用解密后 token 作 HTTP 认证头拉取应下载的 skill 清单，再逐个预签名 URL 下载 zip 到 properties.getLocalDir() → 校验（方式由实现自决）→ 返回 boolean（成功/失败） |
...
**required 字段来源**：第一期 `LocalSkillEntry` 只含 skillId + localPath，无 required 字段。required/optional 语义由 `OpenJiuwenSkillHubProvider` 实现内部决定（如从 Skill Hub 返回的元数据或配置推断），Installer 通过 Provider 提供的 required 标记执行分层失败语义。

**完整性校验数据来源**：`/artifacts/{id}` 响应中的 `checksum_sha256` 和 `file_size` 由 `downloadAndVerify` 实现内部自行决定如何使用（可选 SHA-256 校验或常规校验），校验方式由 Provider 实现自决，文档不强制约束。
...
| required skill 下载/校验失败 | Agent 降级为 ready，skill 不可用；不在请求链路外重试 |
...
| 下载/校验失败降级 | 日志告警 skill 不可用；Agent 继续运行（已降级 ready）；该 skill 永远不注册 |
...
| T2 | required skill 下载失败 | 降级 ready，skill 不可用 | `start()` 正常返回，`Runner.start()` 已调用；skill 未注册 |
| T3 | required skill 校验失败（checksum mismatch） | 降级 ready，skill 不可用 | 同 T2，且 `downloadAndVerify` 返回 false 或抛 `CHECKSUM_MISMATCH`，`listSkills(properties)` 不返回该 skill |
| T4 | 下载成功后注册 | 启动期注册 | `downloadAndVerify` 返回 true；`listSkills(properties)` 返回列表；`registerSkill` 后 skillCount 增长；请求可用到 skill |
| T5 | 校验通过（Provider 用 SHA-256） | digest 校验 | `downloadAndVerify` 返回 true；`listSkills(properties)` 返回含该 skill 的列表，skill 注册成功（校验方式由 Provider 自决，文档不强制 SHA-256） |
| T6 | 校验通过（Provider 用常规检查） | 常规校验兜底 | 同 T5（校验方式由 Provider 自决） |
| T7 | 校验失败拒绝注册 | 不注册未校验材料 | `downloadAndVerify` 返回 false 或抛 `CHECKSUM_MISMATCH`；`listSkills(properties)` 不返回该 skill；required 降级 / optional skip |
| T8 | 首次有效注册后不热刷新 | 幂等保护 | 启动期注册一次，同一 skill 不重复注册 |
...
| `SkillManager` 非线程安全（已确认） | 已核实 agent-core `SkillManager` 的 `registry`（`LinkedHashMap`）/`updateAtCache`（`LinkedHashMap`）/`skillOrder`（`ArrayList`）均非线程安全集合，`register`/`refreshIncrementally`/`clearAll` 等方法无同步保护 | skill 安装只在启动期 `installBeforeStart` 单线程内执行，不涉及并发写；下载/校验失败降级不重试，规避并发问题 |
