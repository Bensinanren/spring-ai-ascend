...
> 命名说明：本设计架构语义使用 L0 逻辑名 `agent-runtime` / `agent-core`。`agent-runtime` 已落地为同名模块（原 `agent-service` 已重命名）；`agent-core` 已落地为 `agent-core`。
...
## 20. Stage 19 决策（重投往返生命周期端到端验证：闭合 Stage 18 盲区）

Stage 18 把一条 record 推进到 `RETRY_SCHEDULED` 就停（单 tick）：证明了重投路径的**入口**（不可达 route → `retry(RECEIVER_UNAVAILABLE)` → `attempt_count=1` + future `next_attempt_at`）。但 `JdbcForwardingOutbox.claimDue` 的 `RETRY_SCHEDULED` 回收子句 `(status='RETRY_SCHEDULED' AND next_attempt_at <= :now)` —— outbox 价值主张的核心（持久化 + 自动重驱动到成功或 DLQ）—— **从未端到端触发**。回收是否真发生、`attempt_count` 是否真在重投间攀升、`ForwardingRetryPolicy` 的 `exhausted` gate 是否在正确计数触发、两出口（exhausted→DLQ / 恢复→ACKED）是否真能闭环，此前只在 fake-delivery 单元 / 契约层覆盖。Stage 19 在真实持久化（embedded-postgres + Flyway + `JdbcForwardingOutbox`）上驱动**同一条 record 越过 `next_attempt_at` 跨多个 tick**，看 `claimDue` 回收它、重投、把 `attempt_count` 推到两个出口之一。`C3ForwardingRetryLifecycleIntegrationTest` 双场景：

### 20.1 场景 A：持续失败 → exhausted → DLQ

不可达 route（`freeUnusedPort()` 真实 socket 拒连）→ 每次 `retry(RECEIVER_UNAVAILABLE)`；`maxAttempts=3` 的小 retry policy 在 4 ticks 耗尽（attempt 0/1/2 retry，attempt 3 exhausted→DLQ）。断言：`claimDue` 真的回收了 `RETRY_SCHEDULED` 行 3 次（`retried==3` + `attempt_count==3` —— 若回收从未触发，`attempt_count` 会卡在 1）；`attempt_count` 停在 `maxAttempts`，因为 `moveToDlq` 不递增（只有 `scheduleRetry` 递增）；终态 DLQ，`last_failure_code = receiver_unavailable`。

### 20.2 场景 B：间歇失败恢复 → ACKED

fake delivery port 注入 `[retry, retry, acked]`（真实 transport-internal 间歇故障恢复模式；Stage 18 把远程 agent 业务 `FAILED` 映射为 `dlq(REMOTE_TASK_FAILED)` —— handler 业务失败无法建模「retry 后恢复」会在 attempt 1 就 DLQ，retryable 失败是 transport 层概念，fake port 直接注入）。DEFAULT policy（`maxAttempts=5`）远离耗尽，恢复才是停下 loop 的原因。断言：`claimDue` 回收 2 次（`claimed==3` = 1 PENDING + 2 RETRY_SCHEDULED reclaim）；`attempt_count==2`（2 次 retry 后恢复，`markAcked` 不递增）；终态 ACKED。

### 20.3 时间控制：MutableEpochClock + 协调多 tick TickSource（核心新增）

跑真实 retry lifecycle 要么 sleep 过 backoff（base 100ms / cap 60s / maxAttempts 5）—— 慢且 flaky。改为 `MutableEpochClock`（test-only 纯 JDK，`epochMillis()` + `advanceTo(instant)`）注入 worker + `advanceableTickSource(clock, baseInstant, stepMillis, ticks)` 生成 N 个 tick instant `baseInstant + i*stepMillis`（`+61s` step > 任何 backoff，故每个 tick 都越过上一次的 `next_attempt_at`）。**两时钟协调**：tick source 在 yield 前 `clock.advanceTo(instant)`，使 worker 的 `clockNow = clock.epochMillis()`（`nextAttemptAt` 基准）与 `claimDue` 的 `:now`（tick instant）同源 —— 这条链最易错的接线点（若两时钟分歧，`claimDue` 回收条件 `next_attempt_at <= :now` 可能永不满足）。

### 20.4 真实时钟约束（leaseGuardedUpdate 用真实墙钟）

`JdbcForwardingOutbox` 的 lease-guarded mutation（`markAcked` / `scheduleRetry` / `moveToDlq`）gate on `lease_until > System.currentTimeMillis()` 用**真实**墙钟（不是注入的那个）。故 tick instant 必须从 `T0`（测试真实起始时刻）单调递增 —— 每次 `leaseUntil = tickInstant + 60s` 始终超过真实时钟，租约防护不误判过期。Stage 19 不改生产代码；真实时钟 lease 语义保持原样。

### 20.5 flaky 修复：context-boot IT 并发 flaky（@Isolated）

parent pom 的 surefire 配 JUnit5 测试类 4 路并发（`mode.classes.default=concurrent`）。Spring Boot 4 `SpringApplication.run` 非线程安全 —— 两个 boot-context 的 `*IntegrationTest`（命名绕过 failsafe 的 `**/*IT.java` include、落到并发 surefire）在同一复用 JVM 撞 `ConcurrentModificationException` + 全局 `spring.autoconfigure.exclude` System property 互相踩踏。Stage 19 给两个 context-boot IT（`C3ForwardingEndToEndIntegrationTest` / `C3ForwardingFailurePathIntegrationTest`）加 JUnit5 `@Isolated`（独占执行、不与其他类并发）根治（parent pom 注释本已警告 boot-context IT 应走串行 failsafe；理想改名 `*IT` 留给 IT owner）。

### 20.6 无生产代码改动 + §6.2 守恒

Stage 19 是纯测试 / 验证阶段。`@Isolated` + import 是测试代码。两场景用 CONTROL_ONLY envelope；`MutableEpochClock` / tick source / fake port 是 test-only 纯 JDK helper。无 concrete broker / payload body / token stream / Task 执行状态 / 跨租户回退 → `§6.2` 文本扫描不触发（helper 在 `src/test`，不进 `readForwardingProductionSources`）。ArchUnit green。

### 20.7 落地清单

- ✓ `C3ForwardingRetryLifecycleIntegrationTest` 场景 A（持续失败→exhausted→DLQ）+ 场景 B（间歇恢复→ACKED）。
- ✓ `MutableEpochClock` + `advanceableTickSource` + `fakeDeliveryPort` test-only helpers（纯 JDK）。
- ✓ flaky 修复：两 context-boot IT 加 `@Isolated`。
- ✓ **186 tests green**（Stage 18 的 184 + 2 retry lifecycle IT），ArchUnit green。
- ✓ §6.2 不变。
- ✓ **deferred**：真实 retry 调度（`ForwardingDispatchLoop` 接真实 scheduler / polling cadence）、真实 agent handler（仍 StubHandler / FailingHandler / fake port）、registry 集成的 resolver 生产实现、连接池治理、push/pull/MQ 最终裁决（仍 H2/H3）。

## 21. Stage 20 决策（lease 过期回收 + 断路器真实链路端到端验证：闭合 Stage 19 两个测试盲区）

Stage 19 驱动重投往返生命周期，验证了 `claimDue` 的 `RETRY_SCHEDULED` 回收子句 `(status='RETRY_SCHEDULED' AND next_attempt_at <= :now)`，但显式留了**两个未验证路径**：

1. **lease 过期（stuck-holder）回收端到端** —— `claimDue` 的第二条回收子句 `(status='DISPATCHING' AND lease_until <= :now)` 解决一个 owning worker 在投递中途崩溃的行（lease 被 dead owner 持有且已过期）。Stage 19 的 advanceable-tick harness 总在行 stuck 在 DISPATCHING 之前就回收了它（每个 tick 重新 claim 一条 RETRY_SCHEDULED 行），故此 stuck-holder 路径仍是单元层未验。
2. **断路器真实链路端到端** —— Stage 16 把 `RouteCircuitBreaker` 接入 worker（§17），但只在单元 / 契约层验证（fake 状态机，无真实持久化 loop）。OPEN 断路器是否真短路一个真实 `claimDue → deliver` tick、是否留行在 DISPATCHING（不消耗 `attempt_count`）、lease 过期后 stuck-holder 回收是否把行交还给 HALF_OPEN 探测并真正恢复，此前全未端到端验证。

Stage 20 在真实持久化（embedded-postgres + Flyway + `JdbcForwardingOutbox`）上闭合这两个盲区。`C3ForwardingLeaseReclaimAndBreakerIntegrationTest` 双场景：

### 21.1 场景 A：lease 过期回收（worker crash 恢复）

enqueue PENDING → SQL fixture 模拟 crash（`UPDATE status='DISPATCHING', lease_owner='worker-dead', lease_until=t0-60s`；CHECK-valid：`ck_outbox_lease_status` 只要求 DISPATCHING 行带非 null `lease_owner`，过期值是普通过去 epoch，非 `RELEASED_LEASE_UNTIL` sentinel）→ 单 tick → `claimDue` 的 stuck-holder 子句在 live owner（`worker-recover`）下回收 → fake port `acked` → ACKED。

断言 `claimed=1`（**唯一**能让 DISPATCHING 行被 claim 的路径就是 lease-expiry 子句——若子句不存在，DISPATCHING 行不在 claimDue 候选集，`claimed` 会是 0）、`attempt_count=0`、`last_failure_code=null`（回收是 fresh delivery，不是 retry，不动计数 / 不留失败码）。

### 21.2 场景 B：断路器全状态机 + lease 回收交织（最高价值测试）

`RouteCircuitBreaker(2, 122_000ms, clock)` + fake delivery port 注入 `[retry, retry, acked]`（间歇 `RECEIVER_UNAVAILABLE` 失败后恢复）+ 4 ticks（`+61s` step 同时 > 任何 backoff 与 60s lease → 每 tick 回收前行）：

- **tick1**（PENDING→DISPATCHING）：breaker CLOSED，deliver retry，`scheduleRetry` → `cf=1`；
- **tick2**（RETRY_SCHEDULED 回收）：breaker CLOSED `cf=1`，deliver retry，`cf=2 ≥ threshold` → **OPEN**（`openedAt=t0+61s`）；
- **tick3**（RETRY_SCHEDULED 回收）：breaker OPEN（`61s < 122s` 冷却）→ **短路 SKIP**，行**留 DISPATCHING**（不 `recordOutcome`、`attempt_count` 保持 2）；
- **tick4**（**stuck-holder lease 过期回收**，DISPATCHING `lease_until=t0+182s ≤ now=t0+183s`）：breaker OPEN（`122s ≥ cooldown`）→ **HALF_OPEN** 单探测（`probeInFlight=true`），deliver acked → **CLOSED**（清零计数、清 `probeInFlight`）→ ACKED。

断言 `claimed=4 / retried=2 / skipped=1 / acked=1`、`breaker.stateOf(route)==CLOSED`、`attempt_count=2`（tick3 skip 不递增）。

**场景 B 是本阶段最高价值测试**：单一 loop 把两条 deferred 路径**交织**在一起 —— 断路器 OPEN 短路正是留行 stuck 在 DISPATCHING 的原因，lease 过期 stuck-holder 回收正是把行交还给 HALF_OPEN 探测的机制。隔离验证会漏掉交互；此测试证明 dispatch loop 的三条 skip 路径（lease 续约失败 / 断路器 OPEN / deliver 异常，见 §17.3）与两条回收子句（RETRY_SCHEDULED / stuck-holder DISPATCHING）在真实 SQL 上正确组合。

### 21.3 时间控制（复用 Stage 19）

`MutableEpochClock` + `advanceableTickSource`（test-only 纯 JDK）复用 Stage 19（§20.3）。`+61s` step 同时超过任何 backoff（cap 60s）与 60s lease，使每个 tick 回收前行：RETRY_SCHEDULED 经 `next_attempt_at <= now`，DISPATCHING 经 stuck-holder `lease_until <= now`。两时钟协调（tick source yield 前 `advanceTo` clock）+ 真实时钟约束（`leaseGuardedUpdate` 用真实墙钟，tick instant 从 `T0` 单调递增）同 Stage 19（§20.4）。场景 A 的单 tick 不依赖协调多 tick，但 `MutableEpochClock` 仍是 worker `clockNow` 的源。

### 21.4 无生产代码改动 + §6.2 守恒

Stage 20 是纯测试 / 验证阶段。`@Isolated`（同 Stage 19，§20.5）+ import + helpers 是测试代码。两场景用 CONTROL_ONLY envelope；`MutableEpochClock` / tick source / fake port 是 test-only 纯 JDK helper。场景 A 的 crash-simulation UPDATE 是 test fixture（无生产代码在无 lease 所有权时写 DISPATCHING 行——生产路径只有 `claimDue` 写 DISPATCHING 且必须带 lease）。无 concrete broker / payload body / token stream / Task 执行状态 / 跨租户回退 → `§6.2` 文本扫描不触发（helper 在 `src/test`，不进 `readForwardingProductionSources`）。ArchUnit green。

### 21.5 落地清单

- ✓ `C3ForwardingLeaseReclaimAndBreakerIntegrationTest` 场景 A（lease 过期回收）+ 场景 B（断路器全状态机 + lease 回收交织）。
- ✓ `simulateCrashedDispatch` test fixture（SQL UPDATE 模拟 worker crash）。
- ✓ 时间控制 helpers 复用 Stage 19（`MutableEpochClock` / `advanceableTickSource` / `fakeDeliveryPort`）。
