...
ForwardingDispatcherWorker (runtime)                // Stage 8：claim / deliver / ack / retry 半边（MI8-003）；Stage 10 增 lease 续约 + skip
  runOnce(tenantId, now, limit, leaseOwner, leaseUntil) -> DispatchTickResult
                                                    ...
                                                    // Stage 10（MI10-002）：DispatchLeasePolicy（构造注入）决定 deliver 前是否 renew
                                                    // Stage 10（MI10-001）：ForwardingLeaseException / renew=false -> skip（skipped），tick 继续
