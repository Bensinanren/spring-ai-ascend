...
- Stage 17 happy-path / Stage 18 失败路径端到端验证（真实 `LocalA2aRuntimeHost`）+ Stage 19 重投往返生命周期端到端验证（真实 `JdbcForwardingOutbox`，claimDue 的 RETRY_SCHEDULED 回收子句跨多 tick 触发 → exhausted/DLQ 或恢复/ACKED）+ Stage 20 验证回填（真实 `JdbcForwardingOutbox`：租约过期卡住持有者回收 `status='DISPATCHING' AND lease_until<=:now` 子句端到端 + 断路器全状态机 CLOSED→OPEN→HALF_OPEN→CLOSED 真实链路端到端）+ Stage 21 多 worker 并发验证（真实 `JdbcForwardingOutbox`：并发 claim 无重复投递端到端 + 共享断路器单例并发一致 OPEN）+ Stage 22 时间驱动的终态与续约端到端验证（真实 `JdbcForwardingOutbox`：EXPIRED 终态 markExpired 落盘 + lease 续约真实触发）+ Stage 23 payloadRef 端到端传递验证（真实 `JdbcForwardingOutbox`：DATA_BEARING payloadRef 在真实 PG 上 enqueue→claim→deliver→ACKED 全程 round-trip 不变；`A2aForwardingDeliveryPort` + MockWebServer：DATA_BEARING 携带 `metadata.payloadRef` / CONTROL_ONLY 省略，两分支对称即 transport 边界 §6.2 data-reference 契约）+ Stage 24 RLS 接线（adapter 首次引入 `TransactionTemplate` 编程式事务，`withTenant` helper 在每端口方法事务内 `set_config('app.tenant_id',:tenantId,true)` ≡ SET LOCAL 激活 §7.3 fail-closed RLS，闭合 Stage 12「armed but not wired」纵深防御债务；双构造器向后兼容、不加 FORCE/WITH CHECK、V1 零改；受限 `app_role` 下 adapter 业务路径 green 作接线 smoking gun），共 200 tests green；agent-bus 加 `agent-runtime` test-scope 依赖、生产仍零依赖。**Stage 26** 落地 broker-agnostic SPI 骨架（`transport.broker` 子包 8 生产文件，纯 Java 不引 broker client）+ 锁定 RocketMQ（首个 broker 生产代码阶段，217 tests green；in-memory 替身 `InMemoryBroker` + 16 契约测试，真实实例 PoC deferred 部署环境）。
- 非生产 in-memory 测试替身仍保留为 fast test double。
- 不改变远端 Task lifecycle owner；`agent-bus` 不写 Task execution state。投递模型已 Stage 25 裁决为 T4 hybrid（outbox + broker，`adopted-t4`，见 [`transport-decision`](../../../docs/architecture/l0/10-governance/review-packets/agent-bus-forwarding-runtime-transport-decision.md)）：保留 outbox + relay produce broker + receiver pull（pull = 反压内核，MQ 本质是 pull），解除 §6.1 第 1 项引 broker（圈 `transport.broker`）、守 §6.2 精神；broker 选型 **Stage 26 锁定 RocketMQ + SPI 骨架落地（`transport.broker` 子包）**；T1 push PoC 保留共存；真实 broker 物理接线 / relay adapter / receiver consumer / `AWAITING_ACK` 状态机 deferred Stage 27+。
