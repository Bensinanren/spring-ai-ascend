...
| 目标态能力 | 职责 | 当前状态 |
|---|---|---|
| 类 MQ 转发底座 | 提供跨 runtime 的异步转发、队列/主题抽象、correlation、ack/retry、backpressure、DLQ/replay、ordering/fairness 等运行时语义。 | Stage 4 broker-agnostic 转发语义 ICD（[`ICD-Agent-Bus-Forwarding`](../../../docs/architecture/l0/05-contracts/human-readable/ICD-agent-bus-forwarding.md)）+ Stage 7 C3 运行态契约（[`ICD-Agent-Bus-Forwarding-Runtime`](../../../docs/architecture/l0/05-contracts/human-readable/ICD-agent-bus-forwarding-runtime.md)）；Stage 7–24 已落：纯 Java 领域模型 / 端口 / 状态机（Stage 7）+ record / claim / lease（Stage 8）+ lease-safe（Stage 9）+ dispatch-loop（Stage 10）+ runtime-completion（Stage 11）+ 真实持久化 Postgres JDBC adapter + Flyway + RLS（Stage 12，打破路径 B）+ transport 候选评审（Stage 13）+ deliver 重投策略 overflow-safe 退避 + exhausted→DLQ（Stage 14）+ 真实 A2A HTTP 投递绑定 PoC（Stage 15，`A2aForwardingDeliveryPort` 消费 agent-runtime `/a2a`）+ 断路器接入 worker 三态机（Stage 16）+ 首次跨模块端到端集成（Stage 17，真实 `LocalA2aRuntimeHost`，happy-path）+ 失败路径端到端 + `REMOTE_TASK_FAILED` 码收口（Stage 18，真实 FAILED→DLQ / 不可达 route→RETRY）+ 重投往返生命周期端到端验证（Stage 19，claimDue 的 RETRY_SCHEDULED 回收子句跨多 tick 触发 → exhausted/DLQ 或恢复/ACKED）+ 验证回填（Stage 20，租约过期卡住持有者回收端到端 + 断路器全状态机真实链路端到端）+ 多 worker 并发验证（Stage 21，并发 claim 无重复投递 + 共享断路器单例并发一致 OPEN）+ 时间驱动的终态与续约端到端验证（Stage 22，EXPIRED 终态 markExpired 落盘 + lease 续约真实触发）+ payloadRef 端到端传递验证（Stage 23，DATA_BEARING payloadRef 真实 PG round-trip 不变 + A2A metadata 传输边界两分支对称，闭合 Stages 17-22 全 CONTROL_ONLY 盲区）+ RLS 接线闭合跨租户纵深防御（Stage 24，adapter 首次引入事务管理 `set_config('app.tenant_id',…,true)` ≡ SET LOCAL 激活 §7.3 fail-closed RLS，闭合 Stage 12「armed but not wired」；有生产代码改动、不加 FORCE/WITH CHECK、V1 零改）；200 tests green（含 real-SQL / transport / 跨模块 IT / retry lifecycle / lease-reclaim-and-breaker / multi-worker-concurrency / expiry-and-lease-renewal / payload-ref-round-trip）；Stage 25 投递模型最终裁决 T4 hybrid（outbox + broker，`adopted-t4`，**无生产代码**）+ **Stage 26 broker-agnostic SPI 骨架（`transport.broker` 子包 8 生产文件，纯 Java 不引 broker client）+ 锁定 RocketMQ（首个 broker 生产代码阶段，217 tests green）**（见 [`transport-decision`](../../../docs/architecture/l0/10-governance/review-packets/agent-bus-forwarding-runtime-transport-decision.md)）：保留 outbox + relay produce broker + receiver pull（pull = 反压内核），解除 §6.1 第 1 项引 broker（圈 `transport.broker`）、守 §6.2 精神；broker 产品 **Stage 26 锁定 RocketMQ**（真实实例 PoC deferred 部署环境）；T1 push PoC 保留共存；真实 broker 物理接线 / relay adapter / receiver consumer / `AWAITING_ACK` 状态机扩展 deferred Stage 27-30。 |
