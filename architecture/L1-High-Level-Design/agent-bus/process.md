...
- 真实 JDBC adapter / Flyway migration / 真实投递绑定（dispatcher worker → receiver transport）仍 deferred（DB / migration / transport 归属经人类再确认为路径 B，Stage 10 / 11：不引入 JDBC / Flyway；真实持久化 deferred 后续阶段）。
