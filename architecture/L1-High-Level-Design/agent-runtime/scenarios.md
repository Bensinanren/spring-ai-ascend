...
runtime 管理 A2A Task、上下文、队列和事件推进；FEAT-003 可把 runtime Task 状态接入 Redis-backed TaskStore。业务 Agent 的 checkpoint、memory 或框架内部状态不由 runtime TaskStore 持久化。
...
- Redis-backed TaskStore 属于 runtime Task 状态存储实现，不改变业务 Agent 状态归属。
...
- Runtime 只桥接上下文；业务 Agent checkpoint 仍归属框架或外部状态能力。
