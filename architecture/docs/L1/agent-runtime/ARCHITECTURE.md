...
| 类型 | 种类 | 语义 | 状态 |
|---|---|---|---|
| `AgentRuntimeHandler` | interface | Agent 执行 SPI：`agentId()` / `isHealthy()` / `execute()` / `resultAdapter()`；卡片元数据钩子 `supportsStreaming()` / `skills()` / `defaultOutputModes()`（默认 fail-safe，handler 显式 opt-in，确保 card 不谎报能力） | shipped |
...
| `ErrorCategory` | enum | OTel-aligned 错误分类，用于轨迹事件 `error.category` 字段（不含 null） | shipped |
| `Redactor` | interface | 可观测性管道扩展点：对载荷字段执行脱敏/遮蔽 | shipped |
| `ValueRecognizingRedactor` | class | `Redactor` 实现：按值内容识别并脱敏 | shipped |
| `PayloadRefStore` | interface | 可观测性管道扩展点：超阈值载荷的引用化存储接口 | shipped |
| `LocalFsPayloadRefStore` | class | `PayloadRefStore` 实现：本地文件系统存储 | shipped |
| `TenantContract` | final class | 租户隔离常量（键名约定） | shipped |
| `AgentCardProvider` | interface | A2A Agent Card 供应 SPI：`describe(): AgentCardDescriptor`，返回中立描述符（ADR-0163，从 `engine.a2a` 迁入） | shipped |
| `AgentCardDescriptor` | record | 中立 Agent Card 数据模型（零 `org.a2aproject` 依赖），覆盖 A2A AgentCard 全字段 | shipped |
| `AgentSkillDescriptor` / `AgentCapabilitiesDescriptor` / `AgentInterfaceDescriptor` / `SecuritySchemeDescriptor` / `SignatureDescriptor` | record | `AgentCardDescriptor` 的中立嵌套类型，逐一镜像对应 A2A 类型，均零 `org.a2aproject` 依赖 | shipped |

> `AgentCardProvider` 现返回中立 `AgentCardDescriptor`（**ADR-0163** 采纳，推翻 ADR-0162 的推迟决策），与描述符类型同处中立 SPI 包；ArchUnit `protocolNeutralPackagesAreA2aSdkFree` 保证这些类型零 A2A SDK 依赖。A2A SDK 类型的构造收敛到唯一投影点 `engine.a2a.A2aAgentCardMapper`（见 §2.B）。
...
| `A2aAgentCardMapper` | final class | **唯一 A2A 投影点**：中立 `AgentCardDescriptor` → A2A SDK `AgentCard`（含 fail-safe 默认；A2A SDK card 类型仅在此构造） | shipped |
| `AgentCards` | final class | 薄描述符工厂，保留 pre-ADR-0163 调用点（`create(...)` 默认 fail-safe 能力；`defaultDescriptor(...)` 返回中立描述符） | shipped |
| `BuildVersion` | final class | 从构建属性/jar manifest 解析 agent-runtime 版本，避免在代码中写死版本号字面量 | shipped |
...
| `OpenJiuwenReActHandler` | ReAct 模式 handler 实现 | shipped |
| `MemoryRuntimeRail` | 顶层类（前版为嵌套类），为 openJiuwen 提供 memory rail 集成 | shipped |
| `OpenJiuwenCheckpointerConfigurer` | Checkpointer 配置适配 | shipped |
| `OpenJiuwenExternalMemoryProviderAdapter` | 外部 MemoryProvider 适配器 | shipped |
| `OpenJiuwenMemoryMessageAdapter` | memory 消息格式适配 | shipped |
| `OpenJiuwenRemoteAgentInterruptRail` | 远端 agent 中断 rail | shipped |
| `OpenJiuwenRemoteToolInstaller` | 远端工具安装器 | shipped |
| `OpenJiuwenTrajectoryRail` | 轨迹 rail 集成 | shipped |
...
| `AgentScopeStatus` | AgentScope 状态枚举/常量 | shipped |
| `AgentScopeErrorCategories` | AgentScope 错误分类常量 | shipped |
...
`RuntimeAutoConfiguration` 已拆分为按关注点隔离的多个配置类：

| 类型 | 语义 | 状态 |
|---|---|---|
| `RuntimeAutoConfiguration` | 顶层装配入口，引用子配置类 | shipped |
| `A2aInfrastructureConfiguration` | A2A SDK 基础设施 Bean（TaskStore / EventBus / QueueManager / RequestHandler / AgentExecutor） | shipped |
| `RuntimeLifecycleConfiguration` | 运行时生命周期管理 Bean | shipped |
| `A2aExecutionConfiguration` | A2A 执行相关 Bean | shipped |
...
| `AgentCardProperties` | `@ConfigurationProperties`：Agent Card 元数据配置（从 `engine.a2a` 迁入 `boot`） | shipped |
| `RemoteAgentProperties` | `@ConfigurationProperties`：远端 Agent 部署配置（从 `engine.a2a` 迁入 `boot`） | shipped |
| `RuntimeAccessProperties` | `@ConfigurationProperties`：运行时接入配置 | shipped |
| `TrajectoryProperties` | `@ConfigurationProperties`：轨迹观测配置 | shipped |
| `AgentRuntimeHealthIndicator` | Actuator 健康检查指示器 | shipped |
| `AgentRuntimeLifecycle` | SmartLifecycle 实现，管理 handler start/stop | shipped |
| `RuntimeReadiness` | 就绪门控（readiness gate） | shipped |
| `TrajectoryOtelConfiguration` | OTel 轨迹导出配置 | shipped |
...
- **Agent Card 发现**: `/.well-known/agent-card.json`（GET → `AgentCard` JSON；带 `Cache-Control` + 内容 `ETag`，支持 `If-None-Match` → 304）
- **Agent Card 兼容路径**: `/.well-known/agent.json`（响应带 `Link: rel="canonical"` 指向规范路径）
...
- **Agent Card 自定义**: 可选实现 `AgentCardProvider.describe()` 返回中立 `AgentCardDescriptor`，否则自动从 `AgentRuntimeHandler.agentId()` + 能力钩子（`supportsStreaming()` / `skills()` / `defaultOutputModes()`）+ `AgentCardProperties` YAML overlay 组装默认 Card。能力字段诚实派生（不谎报 streaming/push）；security 默认不 emit（A2A SDK card resolver 解析限制，opt-in）
