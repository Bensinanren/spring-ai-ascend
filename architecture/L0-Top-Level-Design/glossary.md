...
| TaskStateStore / Historical RunRepository | Controlled read/write entry for Task lifecycle state. If current code or historical documents still use names such as `RunRepository`, read them as implementation-compatible names for the Task owner path, not as a second Run state owner. | `agent-service` Task lifecycle owner | Generic DAO, arbitrary state writer, independent Run state model | compatibility_only |
...
| Business-centric deployment | Deployment where the business side may host `agent-client`, `agent-service`, and `agent-execution-engine`, while the platform may retain shared bus, middleware, and federation governance. | deployment architecture | New module boundary | candidate_promote |
