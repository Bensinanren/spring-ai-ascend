...
| **Principle 2:<br>契约优先与异构中立** | 不绑定特定语言与 Agent 框架。跨模块行为与多体协作必须“契约先行”，先于具体实现被显式描述，并具备仿真验证能力。 | 1. `agent-service` 持有面向异构智能体框架的适配 SPI、反腐装配入口与运行时接入契约，负责吸收外部框架差异。<br>2. 不要求外部开源框架预先满足平台通信契约；平台通过适配实现支持**低改造或免改造运行态接入**。 |
...
| **`agent-sdk`**<br>(智能体开发 SDK) | 面向开发者的智能体开发领域，主流选型为 `openJiuwen`（`agent-core`）。包含 workflow 类智能体的图执行器、ReAct 类智能体的循环执行器，以及可供开发者选用的智能体组件，例如 workflow 不同类型的 Node、agent-loop 不同类型的 Tool、Hook 等。 | 1. `agent-sdk` 提供开发者侧可选用、可组合的智能体开发与执行组件，但不得成为服务端 Task 生命周期所有者。<br>2. 异构智能体框架的运行态兼容主要由 `agent-service` 持有的 SPI 与适配实现完成；`agent-sdk` 可以作为官方实现选型或组件来源被适配接入。 |
...
> 📌 **关于开源实现项目的命名澄清**：
> 未来官方基于 Java 的 `openJiuwen` 实现项目，可能会使用具体的社区项目名称。例如：用 `agent-runtime-java` 表示 `agent-service` 的 Java 实现，用 `agent-core-java` 表示 `agent-sdk` 的 Java 实现。这些 community 名称描述的是**物理实现项目**，不属于新的 L0 逻辑模块。
...
* **策略与约束**：
  * **全链路异步挂起**：基于 `agent-sdk` 与 `agent-service` 之间受治理的执行契约，长等待任务必须持久化至状态机，释放物理线程。
