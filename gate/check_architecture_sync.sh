...
#  --- 2026-05-30 progressive-learning-curve-remediation R2 — status-ledger claim-altitude gate (Rule 151 / kernel Rule G-34; enforcer E201) ---
#  151. status_claim_altitude                            -- CHANGED-FILES-BLOCKING helper gate/lib/check_status_claim_altitude.py: the allowed_claim field of the capability-status ledger docs/governance/architecture-status.yaml is altitude-governed exactly as Rule G-27 governs the L0/L1 prose -- per the authority cascade (generated facts > DSL > Card/prose) the ledger OUTRANKS the prose, so an allowed_claim MUST NOT carry an L1..L8 leaked category (call chains, runtime sequences, SQL/RLS/GUC/persistence, HTTP status/route-verb/header behaviour, filter ordering, wire formats, method signatures, test-class inventories); it should NAME the capability identity + cite its ADR/enforcer (D1/D2/D3) and POINT at the detail's home, not restate it. The helper loads the shared leaked-category vocabulary from docs/governance/layer-purity-policy.yaml and IMPORTS the canonical trigger library (TRIGGERS + the _is_d3_enforcer_citation carve-out) from gate/lib/check_layer_purity.py (one verdict, one probe set), walks every allowed_claim value, and reports a leaked-category trigger not redeemed by a still-open, capability-matched row in the per-surface grandfather list docs/governance/layer-purity-status-ledger-grandfather.yaml. Each grandfather row is CAPABILITY-PRECISE (it pins the ledger capability key it freezes) so a row tolerates exactly one claim and retiring it re-arms the gate on that claim; a ledger-targeting row that omits its capability is a config error (exit 2). It invents no id and no relationship, never outranks a generated fact, does NOT edit the ledger, and does NOT scan the ledger's structural keys (implementation/tests file lists, l0_decision, enforcer rows -- the D2 evidence index). Runs CHANGED-FILES-BLOCKING (--mode changed-files-blocking --base): a PR that edits the ledger may not ADD or worsen a claim-altitude leak; pre-existing (grandfathered) leaks in an untouched ledger stay advisory. An un-importable trigger library, a zero-claim scan, an unknown cited category, or a capability-less ledger row is a config error (exit 2) -- fails closed rather than scan vacuously or with a drifting private probe copy. Ratchet: advisory -> changed-files-blocking (this rung) -> full-blocking, the terminal rung gated by the same clean-workspace + fact-byte-identity condition as Rule G-27 under the ADR-0159 §9 ratchet. A missing helper fails closed; a missing python interpreter is a vacuous pass (Rule G-7 lists WSL as canonical) (Rule G-34, enforcer E201).
