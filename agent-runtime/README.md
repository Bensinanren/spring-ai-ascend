...
- `com.huawei.ascend.runtime.engine.spi.AgentCardProvider` - optional Agent Card metadata provider returning the neutral `AgentCardDescriptor` (zero A2A SDK dependency); keep this separate when the execution handler should stay focused on framework execution and state bridge logic. The sole A2A projection point is `engine.a2a.A2aAgentCardMapper`
