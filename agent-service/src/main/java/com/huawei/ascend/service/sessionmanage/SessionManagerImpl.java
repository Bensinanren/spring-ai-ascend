package com.huawei.ascend.service.sessionmanage;
...
public final class SessionManagerImpl implements SessionManager {
    ...
    public SessionManagerImpl(SessionStatePort sessionStatePort, Clock clock, Duration ttl) {
    ...
    public Session loadOrCreate(String tenantId, String userId, String agentId, String sessionId) {
        ...
        Optional<Session> existing = sessionStatePort.find(key);
        if (existing.isPresent()) {
            return touch(key);
        }
        Instant now = clock.instant();
        Instant expiresAt = ttl == null ? null : now.plus(ttl);
        return sessionStatePort.save(new Session(
                tenantId,
                userId,
                agentId,
                resolvedSessionId,
    ...
    public Optional<Session> get(String tenantId, String sessionId) {
        return sessionStatePort.find(new SessionKey(tenantId, sessionId));
    }

    @Override
    public boolean exists(String tenantId, String sessionId) {
        return sessionStatePort.find(new SessionKey(tenantId, sessionId)).isPresent();
    }

    @Override
    public Session appendMessage(String tenantId, String sessionId, SessionMessage message) {
        Objects.requireNonNull(message, "message");
        return sessionStatePort.update(new SessionKey(tenantId, sessionId), session -> {
    ...
    public void delete(String tenantId, String sessionId) {
        sessionStatePort.remove(new SessionKey(tenantId, sessionId));
    }

    private Session touch(SessionKey key) {
        return sessionStatePort.update(key, session -> withTimestamps(session, session.updatedAt()));
    }
