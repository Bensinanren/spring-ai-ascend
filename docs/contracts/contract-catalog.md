...
| `AgentCardProvider` | `agent-runtime` | `com.huawei.ascend.runtime.engine.spi` | shipped — optional provider for the Agent Card of one runtime-hosted business Agent; returns the neutral `AgentCardDescriptor` (zero `org.a2aproject` dependency) and lives in the framework-neutral SPI package per ADR-0163 (reversing ADR-0162's deferral); A2A SDK card types are constructed only at the sole projection point `engine.a2a.A2aAgentCardMapper`; separated from `AgentRuntimeHandler` so card metadata can be supplied by a dedicated Bean or by a handler that chooses to implement both interfaces |
...
Trajectory observability plumbing — `TrajectoryEmitter`, `TrajectorySource`, `TrajectorySink`, `TrajectorySinkFactory`, `Redactor`, `PayloadRefStore` — is internal **as a Java seam only**: per-invocation, emit-only telemetry interfaces and observability pipeline extension points between the runtime executor and the adapter bases (events are never read back; the runtime stays the source of truth). These interfaces live in `engine.spi` for package-boundary reasons but are not part of the shipped SPI surface (`Redactor` and `PayloadRefStore` are observability pipeline extension points added in the current PR batch). The trajectory's **wire surface is NOT internal**: the A2A metadata keys, the artifact name, and the event payload shape cross the process boundary to A2A callers and are a published contract — see *Northbound trajectory wire contract* below.
...
Schema v3 per-event `DataPart` fields (serialized by `A2aNorthboundSink`; `attempt`/`retryable` exist on the Java record but are not serialized northbound):
...
| `args` / `result` | object \| null | Masked + truncated payloads (`TrajectoryMasking`); may be a `payload_ref://` URI when over-threshold ref-ization is enabled |
| `usage` | object \| null | Token/latency/model telemetry aligned to OpenTelemetry `gen_ai.usage.*`: `{inputTokens, outputTokens, latencyMs, model, provider, inputCostUsd, outputCostUsd}`; `provider` maps to OTel `gen_ai.system`; `inputCostUsd`/`outputCostUsd` support FinOps accounting (nullable, populated by enrichment) |
| `error` | object \| null | `{code, message, category}`; message masked; `category` is an OTel-aligned `ErrorCategory` (never null — use `UNKNOWN` when unmapped) |
| `reasoning` | string \| null | Masked + truncated free-text reasoning |
| `finishReason` | string \| null | Model finish reason (OTel `gen_ai.response.finish_reasons`); populated on `MODEL_CALL_END` when the framework exposes it |
| `parentTaskId` / `parentTraceId` | string \| null | Cross-run linkage: task and trace ids of the invoking parent agent; null for top-level runs |
| `schemaVersion` | string | `"3"` = `TrajectoryEvent.SCHEMA_VERSION` |

**SPI count by module (shipped surface; the agent-runtime SPI surface is `AgentRuntimeHandler` + `AgentCardProvider` + `MemoryProvider` + `StreamAdapter`, all in the framework-neutral `engine.spi` package — `AgentCardProvider` returns the neutral `AgentCardDescriptor` per ADR-0163):**
