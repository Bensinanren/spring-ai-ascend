...
`SessionManager` 是 L2 暴露给其它内部调用方的主入口。其它层不直接访问 `SessionStatePort`，也不依赖具体存储实现。
...
`SessionManagerImpl` 依赖 `SessionStatePort`，不关心它背后是内存、Redis 还是数据库。当前阶段至少提供两个实现：
...
`state` 面向业务恢复；`metadata` 面向协议、追踪和观测。两者都通过 `SessionStatePort.update` 原子更新，避免调用方直接修改 `Session` 内部 Map。
...
调用方 -> SessionManager.delete(tenantId, sessionId)
   -> SessionStatePort.remove(SessionKey)
```
...
| `SessionManagerImpl.java` | Session 操作编排实现，依赖 `SessionStatePort` 完成读写和删除。 |
...
| `RedisSessionStateProperties.java` | Redis Session 存储配置，例如 key prefix、TTL、CAS 最大重试次数、连接配置引用等。 |
...
| 设计对象 | 代码位置 | 对齐说明 |
|---|---|---|
| L2 公共入口 | `service/sessionmanage/SessionManager.java` | 其它内部层只通过该接口访问 Session。 |
| L2 门面实现 | `service/sessionmanage/SessionManagerImpl.java` | 组织 Session 创建、查询、更新、追加消息、写/删状态、写/删元信息、追加 Task 和删除等操作。 |
| L2 Spring 装配 | `service/sessionmanage/config/SessionManageConfiguration.java` | 注册默认内存存储、可选 Redis 存储和 `SessionManager`。 |
| Session 聚合 | `service/sessionmanage/model/Session.java` | 持有消息历史、会话状态和 Task 列表。 |
| Session 存储主键 | `service/sessionmanage/model/SessionKey.java` | 作为 `SessionStatePort` 的查找键。 |
...
| 存储抽象 | `service/sessionmanage/spi/SessionStatePort.java` | 未来替换数据库、Redis 等实现时，上层接口不变。 |
| 内存存储实现 | `service/sessionmanage/memory/InMemorySessionStateAdapter.java` | 本地开发、单机验证和单元测试使用。 |
| Redis 存储实现 | `service/sessionmanage/redis/RedisSessionStateAdapter.java` | 共享环境和生产环境使用，通过配置切换。 |
| Redis 存储配置 | `service/sessionmanage/redis/RedisSessionStateProperties.java` | 定义 key prefix、TTL、CAS 最大重试次数和 Redis 连接配置引用。 |
| Redis 客户端端口 | `service/sessionmanage/redis/RedisSessionCommands.java` | 由具体 Redis client adapter 实现，L2 Redis 存储不直接绑定某个客户端库。 |
| Session 序列化 | `service/sessionmanage/redis/SessionCodec.java` + `JacksonSessionCodec.java` | Redis value 的编码/解码能力，默认使用 Jackson JSON。 |
