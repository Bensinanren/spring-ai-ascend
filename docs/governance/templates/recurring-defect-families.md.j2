...
First observed: rc5. Last observed: progressive-learning-curve-remediation W2 (added two new grandfathered hand-authored factual fields on the EngineeringFrame surface — `saa.cardPath` on all frames, `saa.primaryPackage` on shipped frames per ADR-0161 §2).

Occurrences: rc5, rc7, rc12, rc14, rc15, rc17, rc18, rc19, rc35, rc40, rc48, rc54, rc55, post-W5 L1 Feature Registry, progressive-learning-curve-remediation W2.
...
- `architecture/profile/saa-property-authority.yaml` classifies every `saa.*` key as `intent` / `factual_generated` / `factual_hand_authored_grandfathered` with a sunset date of 2026-07-31 for the third bucket; progressive-learning-curve-remediation W2 registered `saa.cardPath` + `saa.primaryPackage` in this bucket.
- `tools/architecture-workspace/.../facts/` — Java extractor binaries ship in Waves 2-5 (ModuleBuildFactExtractor, AdrFactExtractor, RuntimeConfigFactExtractor, ContractFactExtractor, CodeSymbolFactExtractor via ASM + JavaParser hybrid, TestInventoryFactExtractor); the future EngineeringFrameFactExtractor (ADR-0161 §6 Phase 4-5) will own `saa.cardPath` + `saa.primaryPackage`.
- `gate/lib/check_fact_layer_integrity.py` — provenance + banner + LLM-no-author checks, invoked from gate Rule 131 / E179.

Open residual: the Fact-Layer Round-1 PR shipped the structural foundation — schema + advisory Rule G-15.a + `saa-property-authority.yaml` + ADR-0154. Real extractors land Waves 2-4; the FunctionPoint thicker schema lands W5; sunset of `saa.devPaths` / `saa.verificationTestFqns` / `saa.verificationCommands` runs W6. progressive-learning-curve-remediation W2 ADDED two new grandfathered hand-authored factual fields on the EngineeringFrame surface — `saa.cardPath` (all frames) and `saa.primaryPackage` (shipped frames, ADR-0161 §2) — both under the same `sunset_date: 2026-07-31` commitment, both targeting the future EngineeringFrameFactExtractor. The shipped-frame `saa.primaryPackage` values were derived from `architecture/facts/generated/code-symbols.json` (not invented) but remain hand-authored until that extractor lands, so they can drift if a frame's package root is refactored. Until W6 ships, the legacy + new hand-authored factual fields remain in place; the family stays open with `cleanup_status: partial`. Promotion to `closed` requires (a) one full release cycle with zero new occurrences AND (b) Rule G-15.c blocking (Wave 4 ship) AND (c) the 2026-07-31 sunset date passing with the hand-authored fields removed (or migrated to the frame fact extractor for `saa.cardPath` / `saa.primaryPackage`).
