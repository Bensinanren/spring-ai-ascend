...
> Document-corpus consistency checks for spring-ai-ascend. **164 active gate rules** (canonical bash, executable rule sections counted from `# Rule N — slug` headers), backed by **340 self-tests** (`gate/test_architecture_sync_gate.sh` derives the total at runtime). The canonical numbers live in [`docs/governance/architecture-status.yaml#architecture_sync_gate.baseline_metrics`](../docs/governance/architecture-status.yaml) (single source of truth — Rule G-2 sub-clause .b numeric-agreement check rejects stale counts here; Rule G-5 sub-clause .c enforces `active_gate_checks` AND `enforcer_rows` against live extractors).
>
...
bash gate/test_architecture_sync_gate.sh    # 340 self-tests (~20s); TOTAL derived at runtime per Rule G-5 sub-clause .b; fails closed when passed != TOTAL
...
| `test_architecture_sync_gate.sh` | Self-test harness — 340 self-test cases. `TOTAL` derived at runtime per Rule G-5.b. |
