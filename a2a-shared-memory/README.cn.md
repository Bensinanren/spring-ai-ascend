...
`SharedMemoryStore` / `ExperienceStore` SPI。本模块自带 **in-process 默认实现**(离线可评测;`ScaleTest` 验证 **2000 个并发协作**零跨域泄漏)。生产可换 redis,或闭源 **MemOpt** 引擎(form C 容器 + gRPC,属后端阶段,不在本模块)。
...
**27/27 通过**:A2A 绑定(agent 间按 contextId 共享 + 所有权 + 交接 + 隔离,用真实 agent-runtime context)、黑板(所有权/append-log/并发)、经验(召回/脱敏/租户隔离)、PII redactor、run-end hook、可观测(级别路由/扇出/MDC)、规模(2000 协作并发)。
